...
public class RemoveTripItemHandler(IUnitOfWork unitOfWork, ICurrentUserContext currentUserContext)
    : IRequestHandler<RemoveTripItemCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(RemoveTripItemCommand request, CancellationToken cancellationToken)
    {
        ...
        var tripVault = await unitOfWork.TripVaultRepository.GetByUniqueIdWithSingleItemForUpdateAsync(
        ...
        if (tripVault.QueryHistory.All(i => i.Id != request.ItemId))
        {
            return Result.Failure<bool>(Errors.TripVaultItemNotFound);
        }

        tripVault.RemoveItem(request.ItemId);
