...
public sealed record DeleteScheduledExecutionCommand(
    Guid ScheduledExecutionUniqueId,
    string Username) : IRequest<Result>, IMonitoringService, IAuthorizedRequest
{
    public void IncrementCount(CountMetric countMetric) =>
        countMetric.UpdateMetric(MetricConstants.DeleteScheduledExecution, 1, CountMetric.SetResult(true));

    public void DecrementCount(CountMetric countMetric) =>
        countMetric.UpdateMetric(MetricConstants.DeleteScheduledExecution, 1, CountMetric.SetResult(false));
}
