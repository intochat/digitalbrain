...
public class FlightExecutionStrategy(
    IUnitOfWork unitOfWork,
    IMapper mapper,
    IPreferenceService preferenceService,
    FlightSerpApiHandler flightHandler,
    ITripVaultQuerySaver tripVaultQuerySaver,
    ILogger<FlightExecutionStrategy> logger) : IScheduledExecutionStrategy
{
    public bool CanHandle(ScheduledExecutionSearchType searchType) => Equals(searchType, ScheduledExecutionSearchType.Flights);
    ...
    public async Task<bool> ExecuteAsync(ScheduledExecution scheduledExecution, CancellationToken cancellationToken = default)
    {
        ...
        if (request is null)
        {
            logger.LogInformation("Scheduled flight execution with unique id - {ExecutionId} is not valid, skipping execution", scheduledExecution.UniqueId);
            return false;
        }

        var applied = await preferenceService.AddPreferencesAsync(
            (GetFlightRequestDTO)request,
        ...
        {
            logger.LogError("Failed to apply preferences for scheduled flight execution {ExecutionId}: {Error}",
                scheduledExecution.UniqueId, applied.Error);
            return false;
        }

        await flightHandler.HandleSearchAsync(scheduledExecution, applied.Value!, cancellationToken);

        await SaveQueryHistoryAsync(scheduledExecution, ServiceType.Flight, applied.Value!, cancellationToken);
