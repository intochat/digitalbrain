...
public static class ContainerAppEnvVarHelper
{
        ...
        IEnumerable<EnvironmentVarArgs>? additionalEnvironmentVariables = null)
    {
        ...
        var appDbSecretName = ContainerAppsSecretsHelper.ToKeyVaultSecretName(ServiceConstants.EnvironmentVariables.PostgresConnectionString);
        var hasKeyVaultAppDb = useKeyVaultSecrets && settings.KeyVault?.Secrets.Keys.Any(key =>
            string.Equals(key, appDbSecretName, StringComparison.OrdinalIgnoreCase)) == true;
        ...
        var envVarsList = new List<EnvironmentVarArgs>
        {
            ...
            hasKeyVaultAppDb
                ? new EnvironmentVarArgs
                {
                    ...
                    SecretRef = appDbSecretName.ToLowerInvariant()
                }
                : new EnvironmentVarArgs
                {
                    Name = ServiceConstants.EnvironmentVariables.PostgresConnectionString,
