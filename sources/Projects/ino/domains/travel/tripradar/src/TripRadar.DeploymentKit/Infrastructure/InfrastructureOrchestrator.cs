using TripRadar.DeploymentKit.Exceptions;
using TripRadar.DeploymentKit.Interfaces;
...
namespace TripRadar.DeploymentKit.Infrastructure;
...
public class InfrastructureOrchestrator(ILogger<InfrastructureOrchestrator> logger, ICorrelationIdService correlationIdService, IPreDeploymentValidator preDeploymentValidator, INetworkService networkService, IContainerRegistryService containerRegistryService, IDatabaseService databaseService, ICacheService cacheService, IMonitoringService monitoringService, IStorageService storageService, IContainerAppsService containerAppsService, IKeyVaultService keyVaultService, IApplicationGatewayService applicationGatewayService, IDomainOptimizationService domainOptimizationService, IEventHubsService eventHubsService, IFrontDoorDeployer frontDoorDeployer, ICertificateManagementService? certificateManagementService = null, IDatabaseMigrationService? databaseMigrationService = null)
{
    ...
    /// <summary>
    /// Deploys the complete Trip Radar Server infrastructure
    /// </summary>
    ...
    public async Task<InfrastructureDeploymentOutputs> DeployAsync(InfrastructureSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Starting TripRadar infrastructure deployment with CorrelationId: {CorrelationId} for environment: {Environment} in location: {Location}",
            ...
            _logger.LogInformation(
                "Successfully completed TripRadar infrastructure deployment for environment: {Environment} in {TotalElapsedMs}ms with CorrelationId: {CorrelationId}. Resources created: ResourceGroup={ResourceGroupName}, Database={DatabaseName}, Cache={CacheName}",
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Failed to deploy TripRadar infrastructure for environment: {Environment} after {ElapsedMs}ms with CorrelationId: {CorrelationId}. Error: {ErrorMessage}", settings.Environment, stopwatch.ElapsedMilliseconds, correlationId, ex.Message);
            throw new InfrastructureException($"Infrastructure deployment failed for environment '{settings.Environment}' (CorrelationId: {correlationId})", ex, "TripRadarDeployment", "InfrastructureStack", settings.Environment);
        }
    }
