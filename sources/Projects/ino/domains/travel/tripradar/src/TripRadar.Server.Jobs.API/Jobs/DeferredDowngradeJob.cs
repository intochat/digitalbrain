    ...
    ILogger<DeferredDowngradeJob> logger) : IDeferredDowngradeJob
{
    public async Task ExecuteAsync(long userId, int targetTierId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (result.IsFailure)
            {
                logger.LogError("Failed to process deferred downgrade for user {UserId}: {Error}", userId,
                    result.Error.Reason);
        ...
        {
            logger.LogError(ex, "Unexpected error during deferred downgrade for user {UserId} to tier {TargetTierId}",
                userId, targetTierId);
