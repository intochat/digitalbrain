...
public abstract partial class Agent : IDurableJobHandler
{
    ...
    public virtual async Task ScheduleJob(string name, TimeSpan delay, string prompt, CancellationToken ct = default)
    {
        ...
        var durableJob = await JobManager.ScheduleJobAsync(
            new ScheduleJobRequest { Target = this.GetGrainId(), JobName = name, DueTime = dueTime, Metadata = metadata }, ct);
    ...
    public virtual async Task ExecuteJobAsync(IJobRunContext context, CancellationToken cancellationToken)
    {
        ...
        if (job.IsRecurring)
        {
            ...
            var newDurableJob = await JobManager.ScheduleJobAsync(
                new ScheduleJobRequest { Target = this.GetGrainId(), JobName = jobName, DueTime = nextDueTime, Metadata = metadata }, cancellationToken);
    ...
    private async Task RescheduleExistingJobsAsync(CancellationToken ct)
    {
        foreach (var kvp in durableState.ScheduledJobs)
        {
            ...
            var durableJob = await JobManager.ScheduleJobAsync(
                new ScheduleJobRequest { Target = this.GetGrainId(), JobName = job.Name, DueTime = nextDue, Metadata = metadata }, ct);
