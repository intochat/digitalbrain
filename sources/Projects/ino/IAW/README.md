...
> **IAW is the runtime substrate of [ino](../README.md).** This `iaw/` directory ships as part of the ino solution — its csproj files are members of `D:\ino\ino.slnx`, ino's `AddIno()` delegates to IAW's `AddIAW()`, and ino's `LlmNeuron<TEvent>` inherits `IAW.Core.Agent`. The IAW story below is the standalone description; build and run from the ino root.
...
aspire run                                                      # boots ino on top of the IAW substrate
# Legacy iaw-only AppHost (still works, dormant solution):
dotnet run --project iaw/src/Aspire/Aspire.csproj
```
...
All paths are relative to the ino root (`D:\ino\`). The iaw projects live under `iaw/`:

```
iaw/src/
  Core/                    Agent base class, contracts, LLM integration — inherited by ino's LlmNeuron<TEvent>
  Agents/                  65+ agent implementations
  Agents.CSharp/           Roslyn, DotNet, GitHub, NuGet agents
  Aspire.Hosting/          AppHost extensions (AddIAW, WithLLM<T>) — called by ino's AddIno()
  Aspire.Client/           Service/client registration, OTel
  Aspire/                  Legacy iaw-only AppHost (dormant; ino uses src/Ino.AppHost instead)
  Agents.Host/             Legacy iaw production silo
  MCP/                     MCP server bridge
  DevUI/                   Blazor web UI
  Telegram/                Legacy iaw Telegram bot (ino's clients/Telegram is the active one)
  Testing/                 IAW.Testing — AgentTest<TAgent> with TestCluster
iaw/test/
  Core.Tests/              Agent behavior tests
  Integration.Tests/       Aspire integration tests
  E2E.Tests/               End-to-end tests against the legacy iaw AppHost
```
