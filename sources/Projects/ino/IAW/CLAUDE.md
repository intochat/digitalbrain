...
## Position in the ino tree

This `iaw/` directory is the **runtime substrate of [ino](../README.md)**. iaw is no longer a standalone project — its 14 csproj files are members of `D:\ino\ino.slnx`, and its `Directory.Build.props` + `Directory.Packages.props` were deleted on integration so the whole tree builds against ino's central pins (Aspire 13.2.4, Orleans 10.1.0). `iaw/IAW.slnx` is left in place but dormant: when MSBuild walks up from this directory it finds `D:\ino\Directory.Build.props`, so `dotnet build iaw/IAW.slnx` still works but it picks up ino's settings.

ino consumes iaw two ways:

- **`Aspire.Hosting.IAW`** (`src/Aspire.Hosting`) — `Ino.Aspire.Hosting.AddIno()` calls `builder.AddIAW(name)` and exposes the `IAWService` on `InoBuilder.Iaw`. Silos pick up the cluster + blob + qdrant via `silo.WithReference(ino.Iaw)`.
- **`IAW.Core`** (`src/Core`) — ino's `Ino.Core.Hosting.LlmNeuron<TEvent>` inherits `IAW.Core.Agent`, gaining `IChatClient` streaming + AI tools + tool-approval middleware + durable chat history while layering Neuron's journal-event API on top.

## Build & Test Commands

Run everything from the **ino root** (`D:\ino\`), not from `iaw/`:

```bash
dotnet build ino.slnx                                                 # builds iaw + ino together
dotnet test ino.slnx                                                  # runs ino + IAW.Core.Tests
dotnet test iaw/test/Core.Tests                                       # iaw unit tests only
dotnet test iaw/test/Integration.Tests                                # iaw integration tests
dotnet test --filter "FullyQualifiedName~AgentBasicTests"             # run a single test class
dotnet test --filter "FullyQualifiedName~GetResponse_ReturnsLlmResponse"
```

For an iaw-only AppHost run (the legacy demo), `dotnet run --project iaw/src/Aspire/Aspire.csproj` still works, but the canonical entrypoint is now `aspire run` at the ino root, which boots ino silos on top of the IAW substrate. CI runs on `windows-latest` with .NET 11.0 preview SDK.
...
Every agent is an **Orleans grain** inheriting from `Agent` (abstract, `[GrainType("agent-v3")]`) in `src/Core/Agents/Agent.cs`. ino's `LlmNeuron<TEvent>` (in `D:\ino\src\Ino.Core.Hosting\LlmNeuron.cs`) is the canonical downstream subclass — it adds a journal `IDurableList<EventEnvelope<TEvent>>` for typed event sourcing while keeping every Agent capability. The base class is split across partial files:
...
- Central package versioning lives in **ino's** `D:\ino\Directory.Packages.props` — iaw's was deleted on integration. Add new package pins there, not here.

## API rename watch — Orleans 10.1 DurableJobs

Orleans bumped DurableJobs to `10.1.0-alpha.1` as part of ino's central pins. The IAW agent runtime was migrated:

- `IDurableJobContext` → `IJobRunContext` (the property `Job.Name` survives unchanged)
- `ScheduleJobAsync(grainId, name, dueTime, metadata, ct)` → `ScheduleJobAsync(new ScheduleJobRequest { Target=grainId, JobName=name, DueTime=dueTime, Metadata=metadata }, ct)`

If you hit `CS0246: IDurableJobContext` or `CS1501: No overload for ScheduleJobAsync takes 5 arguments`, you're on the old API — fix per the above.
