...
/// <summary>
/// Hybrid intent dispatcher: regex fast-path against
/// <see cref="INeuronPromptCorpus"/> first, JSON-mode LLM classifier
/// fallback when no pattern hits, and finally <see cref="UnroutedIntent"/>
/// when the model can't pick a candidate. Pinned to the kernel silo so the
/// gateway-to-Cortex hop stays local.
///
/// The corpus is populated from <c>Features/*.feature</c> scenarios tagged
/// with <c>@neuron:&lt;id&gt;</c> — under <c>INO_TEST_MODE</c> it has
/// every scenario the BDD mock knows about, so test routing is purely
/// regex-driven and deterministic. In production the corpus is empty
/// (no .feature loaded) and every prompt goes through the LLM classifier,
/// which the user's configured xAI factory answers in JSON mode.
/// </summary>
[PinToSilo("kernel")]
    ...
    ILogger<CortexNeuron> log) : Grain, INeuron<ChatIntent>
{
    ...
    {
        // NeuronContextSurrogate drops FirePort + Logger on cross-silo hops.
        // Cortex must re-fire, so rebuild against DI-injected instances.
        var liveCtx = ctx with { FirePort = firePort, Logger = log };
        var prompt = synapse.Text;
        ...
        if (await TryFastPathAsync(prompt, neurons, liveCtx, ct) is { } fastResult)
        {
            await RecordRoutingDecisionAsync(synapse.UserId, features, routed: true, ct,
        ...
        if (await PredictWillRouteAsync(synapse.UserId, features, ct) is OptimizationResult pred
            && pred is { Predicted: false, Confidence: >= 0.90f })
        {
            log.LogDebug(
                ...
                synapse.UserId, pred.Confidence);
            await RecordRoutingDecisionAsync(synapse.UserId, features, routed: false, ct,
        ...
        if (await TryClassifyWithLlmAsync(prompt, neurons, liveCtx, ct) is { } llmResult)
        {
            await RecordRoutingDecisionAsync(synapse.UserId, features, routed: true, ct,
    ...
    /// </summary>
    async Task<NeuronResult?> TryFastPathAsync(
        string prompt,
        ...
        CancellationToken ct)
    {
        ...
        foreach (var neuron in neurons)
        {
            ...
            foreach (var pattern in patterns)
            {
                ...
                if (await TryRouteToAsync(neuron, prompt, "regex", pattern.ScenarioName, liveCtx, ct)
                    is { } result)
                    return result;
    ...
    /// Bdd-mock isn't a real classifier, so under <c>INO_TEST_MODE</c> this
        ...
        CancellationToken ct)
    {
        ...
        {
            return await TryRouteToAsync(chosen, prompt, "llm", "json-classifier", liveCtx, ct);
        }
        return null;
    }
    ...
    /// <summary>
    /// Routing tail. Cortex hands control off to the neuron's plan grain
    /// (resolved via <see cref="INeuronDefinition.PlanType"/>) which walks the neuron
    /// graph (BFS over journals + cross-domain fires) and returns a final
    /// <see cref="NeuronResult"/>. Returns null when the canonical handler
    /// isn't installed, the neuron declares no plan, or the plan grain
    /// can't be resolved — the caller then keeps walking the corpus or falls
    /// through to <c>UnroutedIntent</c>.
    ///
    /// As of Phase 4 Slice A there is no single-hop fallback: the legacy
    /// switch over <c>FindFlightsRequest</c>/<c>FindHotelsRequest</c>/etc.
    /// was deleted once each domain shipped a one-hop plan, freeing the
    /// kernel from per-domain synapse types.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        if (neuron.PlanType is { } planType)
            return await TryExecutePlanAsync(neuron, planType, prompt, liveCtx, ct);
    ...
    /// <summary>
    /// Plan-dispatch tail. Resolves the plan grain via <see cref="Grain.GrainFactory"/>
    /// using the user id (or correlation id when no user is bound) as primary
    /// key, then calls <see cref="INeuronPlan.ExecuteAsync"/>. The plan
    /// runs on its declared silo (typically pinned to its owning domain) and
    /// drives the BFS from there. A misconfigured <see cref="INeuronDefinition.PlanType"/>
    /// — one that doesn't extend <see cref="INeuronPlan"/> — logs an error
    /// and returns null so Cortex falls through to <c>UnroutedIntent</c>.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        var key = !string.IsNullOrWhiteSpace(liveCtx.UserId)
            ? liveCtx.UserId
            : liveCtx.CorrelationId.Value;
        ...
        var input = new NeuronPlanContext(prompt, liveCtx with { NeuronId = neuron.Id }, neuron.Id);
        ...
        // interface. We store the AQN rather than an opaque GrainId because
        // the kernel silo's grain reference activator only knows how to
        // materialise references for interfaces in assemblies it loads —
        // generic GrainId resolution fails on cross-silo grain class names
        // the kernel never sees. Plans that don't implement
        // IRfwEventHandler will surface a friendly error at dispatch time.
        if (result.Success && result.Rfw is not null)
        {
            try
            {
                ...
                await registry.RegisterAsync(liveCtx.CorrelationId.Value, aqn, key);
                log.LogInformation(
                    "Cortex: registered RFW correlation {CorrelationId} -> {PlanType}({Key})",
                    liveCtx.CorrelationId.Value, planType.FullName, key);
            }
            catch (Exception ex)
            {
                log.LogWarning(ex,
                    ...
                    liveCtx.CorrelationId.Value, planType.FullName);
            }
        }
    ...
    /// target neuron. Lights up the inspector Reasoning panel via
    /// <see cref="IReasoningProbe"/> on the BddMockChatClient code path. Misses
    /// are advisory — never short-circuit routing.
    /// </summary>
    ...
    async Task<NeuronResult> EmitUnroutedAsync(ChatIntent source, NeuronContext liveCtx, CancellationToken ct)
    {
        var unrouted = new UnroutedIntent(source.Text, source.UserId);
        await firePort.FireBroadcast(unrouted, liveCtx, ct);
        log.LogInformation("Cortex unrouted {Text} for user {UserId}", source.Text, source.UserId);
