...
{
    public const string OtlpForwardClientName = "ino-otlp-forward";

    /// <summary>
    /// Registers the <c>ino-otlp-forward</c> named HttpClient with Aspire service
    /// discovery + the self-signed-cert bypass the dashboard OTLP endpoint needs.
    /// Must be called BEFORE <c>MapInoOtlpProxy</c> (i.e. during builder phase).
    ...
    /// </summary>
    public static IServiceCollection AddInoOtlpForwardClient(this IServiceCollection services)
    {
    ...
    public static IEndpointRouteBuilder MapInoOtlpProxy(this WebApplication app)
    {
        ...
        if (string.IsNullOrWhiteSpace(otlpEndpoint))
        {
            ...
            app.MapPost("/otlp/v1/traces", static ctx => { ctx.Response.StatusCode = 200; return Task.CompletedTask; });
            app.MapPost("/otlp/v1/metrics", static ctx => { ctx.Response.StatusCode = 200; return Task.CompletedTask; });
            app.MapPost("/otlp/v1/logs", static ctx => { ctx.Response.StatusCode = 200; return Task.CompletedTask; });
            return app;
        }
        ...
        else
        {
            ...
            app.MapPost("/otlp/v1/traces", ctx => ForwardAsHttpProtobuf(
                ctx, clientFactory, otlpEndpoint, "v1/traces", otlpHeaders, logger));

            app.MapPost("/otlp/v1/metrics", ctx => ForwardAsHttpProtobuf(
                ctx, clientFactory, otlpEndpoint, "v1/metrics", otlpHeaders, logger));
        }

        app.MapPost("/otlp/v1/logs", ctx => HandleFlutterLogsAsync(ctx, app.Services));
    ...
    static async Task HandleFlutterLogsAsync(HttpContext ctx, IServiceProvider services)
    {
        ...
        // turn each record into an ILogger call tagged "ino-flutter" — that
