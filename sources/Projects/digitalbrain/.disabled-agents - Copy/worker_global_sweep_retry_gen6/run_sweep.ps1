...
$logDir = Join-Path $workspace ".agents\worker_global_sweep_retry_gen6\logs"
...
try {
    ...
    foreach ($proj in $projects) {
        ...
        $progressContent = @"
# Progress — worker_global_sweep_retry_gen6

Last visited: $timestamp

## Tasks
- [x] Clean background processes (Stop-Process) <!-- id: 0 -->
- [x] Copy optimized ``run_sweep.ps1`` from worker_global_sweep_retry_gen5 <!-- id: 1 -->
...
"@
        Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry_gen6\progress.md") -Value $progressContent
...
Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry_gen6\sweep_results.json") -Value $resultsJson
