...
$logDir = Join-Path $workspace ".agents\worker_global_sweep_retry\logs"
...
function Stop-LingeringProcesses {
    ...
    cmd.exe /c "dotnet build-server shutdown > $null 2>&1"
...
try {
    ...
    foreach ($proj in $projects) {
        ...
        $progressContent = @"
# Progress - Final Global Test Sweep

Last visited: $timestamp

- [x] Clean background processes and build servers <!-- id: 0 -->
- [x] Copy and adapt the global sweep script run_sweep.ps1 <!-- id: 1 -->
- [ ] Sequentially run the 23 test projects using run_sweep.ps1 <!-- id: 2 --> (Running ${currentCount} of ${totalProjects} - ${projName})
- [ ] Verify that 100% of all tests pass <!-- id: 3 -->
- [ ] Create comprehensive handoff.md report <!-- id: 4 -->
- [ ] Notify the parent orchestrator via message <!-- id: 5 -->
"@
        Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry\progress.md") -Value $progressContent
        ...
        $isModern = ($projContent -like "*UseMicrosoftTestingPlatformRunner*") -or ($projContent -like "*xunit.v3*") -or ($projContent -like "*Testing.Platform*")
        ...
        # Build using single-process configuration (/m:1) to completely avoid MSBuild multi-node lock clashes
        ...
        # Run test with no-build, omitting MSBuild properties to prevent Testing Platform command-line parser failures
        ...
        $status = "FAIL"
        if ($failed -eq 0 -and $foundSummary -and $passed -gt 0) {
            $status = "PASS"
        }
...
Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry\sweep_results.json") -Value $resultsJson
