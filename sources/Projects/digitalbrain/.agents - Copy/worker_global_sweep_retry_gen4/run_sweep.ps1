...
$logDir = Join-Path $workspace ".agents\worker_global_sweep_retry_gen4\logs"
...
try {
    ...
    foreach ($proj in $projects) {
        ...
        $progressContent = @"
# Progress — worker_global_sweep_retry_gen4
...
- [/] Run sequential test sweep script <!-- id: 2 --> (Running ${currentCount} of ${totalProjects} - ${projName})
- [ ] Inspect test logs and ``sweep_results.json`` <!-- id: 3 -->
- [ ] Ensure all active test projects pass cleanly <!-- id: 4 -->
- [ ] Create ``changes.md`` and ``handoff.md`` <!-- id: 5 -->
- [ ] Send handoff message to caller Project Orchestrator <!-- id: 6 -->
"@
        Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry_gen4\progress.md") -Value $progressContent
...
Set-Content -Path (Join-Path $workspace ".agents\worker_global_sweep_retry_gen4\sweep_results.json") -Value $resultsJson
