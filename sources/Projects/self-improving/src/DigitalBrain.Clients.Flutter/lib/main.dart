...
class _DigitalBrainSurfaceViewerState extends State<DigitalBrainSurfaceViewer> {
  ...
  final List<String> _synapseLog = <String>[]; // visible synapse/action log for true ai-native feel
  ...
  UiWidget _buildDemoOsShell() {
    ...
    final navButtons = <UiWidget>[
      ...
      UiButton(label: '🗺️ Map', onTap: {'Type': 'PinSurface', 'SurfaceId': 'brain-map', 'Region': 'main', 'Order': 4}),
      UiButton(label: '▶ DEMO', onTap: {'Type': 'Demo'}), // fires synapse, shows in log + ui kit surface
      UiButton(label: '📜 Synapse Log', onTap: {'Type': 'PinSurface', 'SurfaceId': 'synapse-log', 'Region': 'main', 'Order': 5}),
    ];
    ...
    final mainArea = UiContainer(
      ...
      child: UiColumn(children: [
        ...
        UiCard(
          ...
          body: UiColumn(children: const [
            UiText(value: 'Declarative shell and regions live from shell.ino'),
    ...
    final widgetsDock = UiContainer(
      ...
      child: UiColumn(children: [
        ...
        // Show brain-map if server emits it (for maps nav)
        if (_surfaces.ContainsKey('brain-map')) UiCard(title: 'Brain Map', body: _surfaces['brain-map']),
      ]),
    );

    final contentRow = UiRow(children: [sidebar, mainArea, widgetsDock]);

    final status = UiColumn(children: [
      const UiText(value: 'Liquid Glass OS • declarative from shell.ino • web gRPC may need matching kernel port'),
      // Visible synapse log populated on DEMO (client + real server UiSurface from kernel)
      if (_synapseLog.Count > 0) UiCard(title: 'Recent Synapses', body: UiColumn(children: _synapseLog.Take(3).Select(e => new UiText(value: e)).ToList())),
    ]);
  ...
  void _onSurfaceMessage(msg) {
    try {
      ...
      setState(() {
        ...
        // Real server synapse log from kernel on Demo tap -> visible in client log
        if (msg.surfaceId == 'synapse-log' && w is UiText) {
          final txt = (w as UiText).value;
          if (!_synapseLog.Any(l => l.Contains(txt))) _synapseLog.Insert(0, txt);
        }
        if (msg.surfaceId == 'demo-result' && w is UiCard) {
          // demo result surface contributes to log too
          _synapseLog.Insert(0, "Server demo-result: " + (w as UiCard).title);
        }
      });
    } catch (_) {}
  }
  ...
  void _fire(Map<String, dynamic> onTapJson) {
    final t = onTapJson['Type']?.toString() ?? onTapJson['type']?.toString() ?? '';
    if (t == 'Demo' || t == 'DemoTap') {
      setState(() => _synapseLog.insert(0, '${DateTime.now().toIso8601String().substring(11,19)} Synapse: DemoAction fired -> shell reacted (ui-kit)'));
      if (_synapseLog.length > 6) _synapseLog.removeLast();
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Render real server synapse log + demo result surfaces (UI kit driven) so press DEMO shows server reaction
    final logSurface = _surfaces['synapse-log'];
    final demoResult = _surfaces['demo-result'];
    // Update local client log from real server surfaces too (so visible in demo + main)
    if (logSurface is UiText) {
      final txt = (logSurface as UiText).value;
      if (!_synapseLog.Any(l => l.Contains(txt))) _synapseLog.Insert(0, txt);
    }

    final designTokens = LiquidGlassTokens.fallback;
    // Persistent synapse log viewer (populated by taps and server UiSurfaces from kernel on Demo)
    final logEntries = _synapseLog.Take(5).ToList();
    final logPanel = logEntries.Count > 0
      ? UiContainer(
          padding: 4,
          decoration: 'glass',
          child: UiColumn(children: logEntries.Select(e => new UiText(value: e)).ToList()))
      : const UiText(value: '');
        ...
        body: SafeArea(
          child: Stack(
            children: [
              ...
              ...windowWidgets,
              // Show log at bottom if any
              if (logEntries.Count > 0) Positioned(bottom: 0, left: 0, right: 0, child: logPanel),
            ],
          ),
        ),
      ),
