...
using DigitalBrain.Aspire.Hosting; // for LLMAttribute<T> + model markers
using Microsoft.Extensions.AI; // IChatClient for [LLM<T>] expressive injection (default = Gemma4_26b local Ollama)
...
public sealed class CreatorNeuron : Neuron, ICreatorNeuron
{
    ...
    public CreatorNeuron(
        ...
        // Default IChatClient is the expressive local LLM (Gemma4_26b via Ollama).
        // [LLM<Gemma4_26b>] IChatClient makes the intent explicit and allows future model-specific resolution.
        [LLM<Gemma4_26b>] IChatClient? llm = null)
        : base()
    {
        _brainOverride = brain;
        // llm can be used for proposal generation / self-improvement loops + MCP tool calling against the brain.
        _ = llm; // wired as default LLM (real MCP integration for tool use against self-improving system can be added here)
    }
