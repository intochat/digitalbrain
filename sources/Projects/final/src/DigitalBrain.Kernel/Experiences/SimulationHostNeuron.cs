...
public sealed class SimulationHostNeuron : Neuron, IHandle<RunSimulation>
{
    ...
    public async Task HandleAsync(RunSimulation synapse, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(inoId))
        {
            ...
            var replay = await ruleHost.ReplayObservedSynapsesAsync(manifest, cancellationToken);
            var outcome = replay.Faults.Length == 0 ? "Passed" : "Failed";
            var diag = replay.Faults.Length > 0 ? string.Join("; ", replay.Faults.Select(f => f.Message)) : "";
