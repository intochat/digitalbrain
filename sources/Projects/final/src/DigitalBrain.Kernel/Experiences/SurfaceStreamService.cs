...
public sealed class SurfaceStreamService : DigitalBrain.Surfaces.SurfaceStream.SurfaceStreamBase
{
    private static readonly ConcurrentDictionary<IServerStreamWriter<DigitalBrain.Surfaces.UiSurfaceMessage>, string> Writers = new();
    private static readonly ConcurrentDictionary<string, DigitalBrain.Surfaces.UiSurfaceMessage> LastSurfaces = new();
    ...
    public override async Task SubscribeSurfaces(DigitalBrain.Surfaces.SurfaceSubscription request, IServerStreamWriter<DigitalBrain.Surfaces.UiSurfaceMessage> responseStream, ServerCallContext context)
    {
        ...
        DigitalBrain.Os.SurfaceFanout.Instance = new KernelSurfaceFanout();
        var seed = new DigitalBrain.Surfaces.UiSurfaceMessage
        {
    ...
    public override async Task<DigitalBrain.Surfaces.ClientEventResponse> SendClientEvent(DigitalBrain.Surfaces.ClientEvent request, ServerCallContext context)
    {
        ...
        if (gf != null && !string.IsNullOrWhiteSpace(request.PayloadJson))
        {
            try
            {
                ...
                if (request.PayloadJson.Contains("Demo", StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine("[SurfaceStream] Demo ClientTap delivered -> grain should have emitted UiSurface(demo-executed)");
                    Console.WriteLine("[fire-demo] gRPC success=True -> UiSurface should be emitted");
        ...
        return new DigitalBrain.Surfaces.ClientEventResponse
        {
            Success = true,
    ...
    public static void Publish(DigitalBrain.Surfaces.UiSurfaceMessage message)
    {
    ...
    public static DigitalBrain.Surfaces.UiSurfaceMessage ToMessage(UiSurface surface) =>
        new()
        {
    ...
    private sealed class KernelSurfaceFanout : DigitalBrain.Os.ISurfaceFanout
    {
        public void Publish(DigitalBrain.Os.UI.UiSurface surface)
        {
            // research fanout disabled (stray cleaned); surfaces travel timeline only for core InoLang + TUI
        }
    }

    public override async Task<DigitalBrain.Surfaces.LoginResponse> Login(DigitalBrain.Surfaces.LoginRequest request, ServerCallContext context)
    {
        var gf = _grainFactory ?? _serviceProvider?.GetService<Orleans.IGrainFactory>();
        var resp = new DigitalBrain.Surfaces.LoginResponse { Username = request.Username };
        if (gf != null)
        {
    ...
    public override async Task<DigitalBrain.Surfaces.BrainDescriptor> AddBrain(DigitalBrain.Surfaces.AddBrainRequest request, ServerCallContext context)
    {
        var gf = _grainFactory ?? _serviceProvider?.GetService<Orleans.IGrainFactory>();
        if (gf != null)
        {
        ...
        var d = new DigitalBrain.Surfaces.BrainDescriptor { Name = request.BrainName, Kind = "GrainKeyed" };
        try
        {
            var br = gf.GetGrain<DigitalBrain.Os.Application.IDigitalBrain>(request.BrainName);
    ...
    public override async Task<DigitalBrain.Surfaces.ClientEventResponse> ArchiveBrain(DigitalBrain.Surfaces.ArchiveBrainRequest request, ServerCallContext context)
    {
        var gf = _grainFactory ?? _serviceProvider?.GetService<Orleans.IGrainFactory>();
        if (gf != null)
        {
        ...
        return new DigitalBrain.Surfaces.ClientEventResponse { Success = true };
    }
}