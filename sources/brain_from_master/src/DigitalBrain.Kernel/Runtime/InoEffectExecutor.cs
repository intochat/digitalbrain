...
internal sealed class InoEffectExecutor : IInoEffectExecutor
{
    ...
    public Task<InoToolEffectResult> ExecuteAsync(InoToolEffectRequest request, CancellationToken cancellationToken = default)
    {
        if (!_tools.Contains(request.ToolId) ||
            !_authority.TryValidateToken(request.Scope, request.ActorScope, request.ToolId, out var planId, out var summaryDigest))
            return Task.FromResult(new InoToolEffectResult(InoToolEffectDisposition.Failed, "The approved action did not have a valid typed execution plan. No external action was performed."));
        var executionProof = _authority.IssueExecutionProof(planId, request.ActorScope, request.OperationId, request.ToolId, request.EffectId, request.ProviderIdempotencyKey);
