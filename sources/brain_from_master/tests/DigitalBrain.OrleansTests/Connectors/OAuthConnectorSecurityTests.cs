...
public sealed class OAuthConnectorSecurityTests
{
    ...
    public void Google_authorization_requests_only_readonly_and_send_gmail_scopes()
    {
        ...
        Assert.Equal(
            ...
            new GoogleConnector(new FakeIntegrationConfigStore(), new FakeOAuthStateProtector()).Descriptor.Scopes);
    ...
    {
        var store = new FakeIntegrationConfigStore();
        await store.SetAsync(IntegrationConfigScopes.App, SalesforceClientFactory.PackName, SalesforceAppConfig());
        var handler = new StubTokenEndpointHandler(
    ...
    public async Task Salesforce_oauth_pins_login_and_redirect_for_the_started_flow()
    {
        ...
        await store.SetAsync(IntegrationConfigScopes.App, SalesforceClientFactory.PackName, initial);
        var handler = new StubTokenEndpointHandler(
            "{\"access_token\":\"access-a\",\"instance_url\":\"https://example.my.salesforce.com\",\"refresh_token\":\"refresh-a\"}");
        var connector = new SalesforceConnector(
            new FakeSalesforceApiClientFactory(),
        ...
        await store.SetAsync(IntegrationConfigScopes.App, SalesforceClientFactory.PackName, changed);
    ...
    private static string UserScope(NeuronId owner) => IntegrationConfigScopes.ForUser(new UserId(owner.Value));
    ...
    private sealed class PendingClearFailureStore : IIntegrationConfigStore
    {
