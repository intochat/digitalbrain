...
{
    public async ValueTask<CapabilityGrant?> ReadAsync(CapabilityRequest request, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);
        var hub = grains.GetGrain<IFeatureHubGrain>(FeatureGrainIds.Hub(request.OwnerId));
        var snapshot = await hub.ReadGrantAsync(new FeatureGrantLookup(request.InstallationId, request.ReleaseDigest, request.CapabilityId, request.CapabilityVersion))
            .WaitAsync(cancellationToken)
            .ConfigureAwait(false);
        if (snapshot is null || snapshot.ActorId != request.ActorId || snapshot.Revision != request.GrantRevision ||
