...
{
    public static FeatureHubState Propose(
        ...
        DateTimeOffset now)
    {
        ...
        if (!grants.Select(grant => grant.CapabilityId).Order(StringComparer.Ordinal).SequenceEqual(
                release.RequestedCapabilities.Order(StringComparer.Ordinal),
        ...
        var existingApproval = state.Approvals.FirstOrDefault(candidate =>
            ...
            candidate.Release.Digest == release.Digest);
    ...
    public static FeatureHubState Decide(
        FeatureHubState state,
    ...
    public static FeatureHubState Grant(
        FeatureHubState state,
        ...
        long expectedRevision)
    {
        ...
        var approval = state.Approvals.LastOrDefault(candidate =>
            ...
            candidate.Release.Digest == request.Release &&
        ...
        var authority = (current ?? new FeatureInstallationAuthorityState(
    ...
    public static FeatureHubState Activate(
        FeatureHubState state,
        ...
        long expectedRevision)
    {
        ...
        if (authority.PendingRelease is not { } pendingRelease ||
            authority.PendingGrantRevision is not { } pendingRevision)
    ...
    public static FeatureHubState PauseAuthority(
        FeatureHubState state,
        FeatureInstallationId installationId,
        string reason,
        long expectedRevision)
    ...
    public static FeatureHubState ResumeAuthority(
        FeatureHubState state,
        FeatureInstallationId installationId,
        long expectedRevision)
    {
    ...
    public static FeatureHubState Revoke(
        FeatureHubState state,
    ...
    public static FeatureHubState RollbackAuthority(
        FeatureHubState state,
        FeatureInstallationId installationId,
        long expectedRevision)
    {
        ...
        var index = AuthorityIndex(state, installationId);
        var authority = state.Authorities[index];
        if (authority.PreviousRelease is not { } previousRelease ||
            authority.PreviousGrantRevision is not { } previousRevision)
            return state;
        var authorities = state.Authorities.ToArray();
        authorities[index] = authority with
        {
    ...
    public static FeatureGrantState? ReadGrant(FeatureHubState state, FeatureGrantLookup lookup)
    {
        ...
        return authority.PreviousRelease == lookup.Release
            ? FindGrant(authority.PreviousGrants, lookup)
            : null;
    }
    ...
    private static FeatureReleaseMetadata ValidateRelease(FeatureReleaseMetadata release)
    {
        ...
        if (!release.SourceReference.StartsWith("sha256:", StringComparison.Ordinal) ||
            release.SourceReference.Length != 71 ||
    ...
    private static FeatureGrantState[] ValidateGrants(FeatureGrantSpec[] grants)
    {
        ...
        return grants.Select(grant =>
        {
            ...
            if (string.IsNullOrWhiteSpace(grant.CapabilityId) || grant.CapabilityId.Length > 256 ||
                grant.CapabilityId.Any(char.IsControl) || grant.CapabilityVersion < 1 ||
    ...
    private static bool SameGrants(
        IReadOnlyList<FeatureGrantState> left,
        IReadOnlyList<FeatureGrantState> right) =>
    ...
    private static string[] CanonicalValues(string[] values, string kind)
    {
        ...
        if (values.Length > 64 || values.Any(value => string.IsNullOrWhiteSpace(value) || value.Length > 256 ||
    ...
    private static string ApprovalId(
        ...
        long revision) => Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes(
        $"digitalbrain.v3.feature-approval\0{installationId.Value}\0{release.Value}\0{revision}")));
    ...
    private static FeatureInstallationAuthorityState RemoveGrant(
