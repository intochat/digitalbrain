...
public interface IPrivateFeedStore
{
    StoredSurfaceRecord Append(
    ...
    StoredSurfaceRecord EnsureInitial(
        RequestContext context,
        SurfaceAudienceKind audienceKind,
        string surfaceId,
        Func<long, StoredSurfaceRecord> factory);

    FeedPage CatchUp(RequestContext context, SurfaceAudienceKind audienceKind, long? after, int limit = 50);
    ValueTask WaitForChangeAsync(RequestContext context, SurfaceAudienceKind audienceKind, long after, CancellationToken cancellationToken);
    void RetainFrom(RequestContext context, SurfaceAudienceKind audienceKind, long minimumSequence);
    void MarkDelivered(RequestContext context, SurfaceAudienceKind audienceKind, long sequence);
    ValueTask<bool> WaitUntilDeliveredAsync(RequestContext context, SurfaceAudienceKind audienceKind, long sequence, CancellationToken cancellationToken);
    void Acknowledge(RequestContext context, SurfaceAudienceKind audienceKind, long sequence);
    long? Acknowledged(RequestContext context, SurfaceAudienceKind audienceKind);
    int? LatestRevision(RequestContext context, SurfaceAudienceKind audienceKind, string surfaceId);
}
...
/// </summary>
public sealed class PrivateFeedStore : IPrivateFeedStore
{
    ...
    public PrivateFeedStore(string? storagePath = null, Action<string>? appendLine = null, byte[]? integrityKey = null)
    {
        _storagePath = string.IsNullOrWhiteSpace(storagePath) ? null : Path.GetFullPath(storagePath);
        if (integrityKey is { Length: < 32 }) throw new ArgumentException("The feed integrity key must be at least 256 bits.", nameof(integrityKey));
    ...
    public StoredSurfaceRecord Append(
        RequestContext context,
        SurfaceAudienceKind audienceKind,
        string surfaceId,
        int revision,
        ...
        int actionSchemaVersion = UiProtocol.ActionSchemaVersion)
    {
        ...
        lock (state.Gate)
        {
            ...
            if (current is not null && revision <= current.Revision)
                throw new InvalidOperationException("Surface revisions must increase monotonically.");
            if (current is null && state.Current.Count >= MaximumActiveSurfacesPerAudience)
                throw new InvalidOperationException("The feed active-surface bound has been reached.");
            var sequence = checked(state.LastSequence + 1);
            record = new StoredSurfaceRecord(
                sequence,
                context.TenantId,
                context.WorkspaceId,
                ...
                actionSchemaVersion,
                audienceKind == SurfaceAudienceKind.Principal ? context.Principal.Kind : null);
    ...
    public StoredSurfaceRecord EnsureInitial(
        RequestContext context,
        SurfaceAudienceKind audienceKind,
        string surfaceId,
        Func<long, StoredSurfaceRecord> factory)
    {
        ...
        var key = Key(context, audienceKind);
        var state = _feeds.GetOrAdd(key, static _ => new FeedState());
        StoredSurfaceRecord? created = null;
    ...
    // Compatibility overload retained for existing unit consumers; new code must select an audience explicitly.
    public StoredSurfaceRecord Append(RequestContext context, string surfaceId, int revision, string contentHash, JsonElement payload)
    {
        var compatibleHash = IsHash(contentHash)
            ? contentHash
            : SurfaceContentHash.Compute(payload, []);
        return Append(context, SurfaceAudienceKind.Workspace, surfaceId, revision, compatibleHash, DateTimeOffset.UtcNow, null,
            context.CorrelationId, "surface", surfaceId, [], payload, []);
    }

    public FeedPage CatchUp(RequestContext context, SurfaceAudienceKind audienceKind, long? after, int limit = 50)
    {
        ...
        lock (state.Gate)
        {
            ...
            var items = new List<StoredSurfaceRecord>(Math.Min(take, state.Items.Count - start));
            var index = start;
            ...
            var next = items.Count > 0 && items[^1].Sequence < latest
                ? new FeedCursor(items[^1].Sequence, CursorNonce(context, audienceKind, items[^1].Sequence))
                : null;
            return new(items.ToArray(), next, false, false, latest);
        }
    }

    private static int FirstSequenceAfter(List<StoredSurfaceRecord> items, long sequence)
    {
        var low = 0;
    ...
    private static FeedPage SnapshotPage(FeedState state, long latest)
    {
        var now = DateTimeOffset.UtcNow;
    ...
    public FeedPage CatchUp(RequestContext context, long? after, int limit = 50) =>
        CatchUp(context, SurfaceAudienceKind.Workspace, after, limit);
    ...
    public void RetainFrom(RequestContext context, SurfaceAudienceKind audienceKind, long minimumSequence)
    {
    ...
    public void RetainFrom(RequestContext context, long minimumSequence) =>
        RetainFrom(context, SurfaceAudienceKind.Workspace, minimumSequence);

    public void Acknowledge(RequestContext context, SurfaceAudienceKind audienceKind, long sequence)
    {
        if (sequence < 0) throw new ArgumentOutOfRangeException(nameof(sequence));
        var key = Ack(context, audienceKind);
        if (!_delivered.TryGetValue(key, out var delivered) || sequence > delivered)
            throw new InvalidOperationException("Cannot acknowledge an unseen feed sequence.");
        _acks.AddOrUpdate(key, sequence, (_, current) => Math.Max(current, sequence));
    }

    public void MarkDelivered(RequestContext context, SurfaceAudienceKind audienceKind, long sequence)
    {
        if (sequence < 0 || sequence > LatestSequence(context, audienceKind))
            throw new InvalidOperationException("Cannot record delivery outside the feed watermark.");
        var key = Ack(context, audienceKind);
    ...
    public void Acknowledge(RequestContext context, long sequence) =>
        Acknowledge(context, SurfaceAudienceKind.Workspace, sequence);

    public long? Acknowledged(RequestContext context, SurfaceAudienceKind audienceKind) =>
        _acks.TryGetValue(Ack(context, audienceKind), out var sequence) ? sequence : null;

    public long? Acknowledged(RequestContext context) => Acknowledged(context, SurfaceAudienceKind.Workspace);

    public int? LatestRevision(RequestContext context, SurfaceAudienceKind audienceKind, string surfaceId)
    {
    ...
    private long LatestSequence(RequestContext context, SurfaceAudienceKind audienceKind)
    {
    ...
    private void Load()
    {
        ...
        foreach (var journalLine in JsonLinesJournalFile.ReadCompleteLines(_storagePath).Lines)
        {
            var lineNumber = journalLine.LineNumber;
            var line = journalLine.Text;
            ...
            catch (JsonException)
            {
                ...
                throw new InvalidDataException($"The UI feed journal contains invalid JSON at line {lineNumber}.");
            }
            if (entry is null || !VerifyIntegrity(entry))
            {
                Quarantine(lineNumber, line, "invalid-integrity");
                throw new InvalidDataException($"The UI feed journal failed integrity validation at line {lineNumber}.");
            }
            if (entry?.Kind == "append" && entry.Record is not null && TryValidateLoadedRecord(entry.Record))
            {
                ...
                lock (state.Gate)
                {
                        ...
                        current is null && state.Current.Count >= MaximumActiveSurfacesPerAudience)
                    {
                        ...
                        throw new InvalidDataException($"The UI feed journal contains an invalid sequence or revision at line {lineNumber}.");
                    }
            ...
            else
            {
                ...
                throw new InvalidDataException($"The UI feed journal contains an invalid record at line {lineNumber}.");
            }
        }
    }
    ...
    private void Persist(FeedLogEntry entry)
    {
        ...
        {
            using var exclusion = _storagePath is null
                ? null
                : JsonLinesJournalFile.AcquireWriterExclusion(_storagePath);
    ...
    private static bool TryValidateLoadedRecord(StoredSurfaceRecord record)
    {
        try
        {
            ...
            var recomputed = SurfaceContentHash.Compute(record.Payload, record.Actions);
    ...
    private static bool ValidAudience(
        SurfaceAudienceKind kind,
        string id,
        string workspace,
        PrincipalKind? principalKind) => kind switch
        {
            SurfaceAudienceKind.Principal => IsBoundedId(id) && principalKind is not null && Enum.IsDefined(principalKind.Value),
            SurfaceAudienceKind.Workspace => principalKind is null && string.Equals(id, workspace, StringComparison.Ordinal),
            SurfaceAudienceKind.Public => principalKind is null && id.Length == 0,
            _ => false
        };

    private static void ValidateOwnedRecord(RequestContext context, SurfaceAudienceKind audienceKind, string surfaceId, long sequence, StoredSurfaceRecord record)
    {
        if (record.Sequence != sequence || record.TenantId != context.TenantId || record.WorkspaceId != context.WorkspaceId ||
            ...
            record.AudiencePrincipalKind != (audienceKind == SurfaceAudienceKind.Principal ? context.Principal.Kind : null) ||
            !string.Equals(record.SurfaceId, surfaceId, StringComparison.Ordinal))
            throw new InvalidOperationException("The initial surface factory attempted to escape its authenticated scope.");
    }
    ...
    private static void ValidateContext(RequestContext context)
    {
        if (string.IsNullOrWhiteSpace(context.TenantId.Value) || context.TenantId.Value.Length > 256 ||
            ...
            throw new ArgumentException("Feed scope identifiers must be present and bounded.", nameof(context));
    }
    ...
    private static void ValidateRecordInput(string surfaceId, int revision, string contentHash, string correlationId,
        ...
        IReadOnlyList<StoredActionBinding> actions, int protocolVersion, string surfaceSchema, int surfaceSchemaVersion,
        int actionSchemaVersion)
    {
        if (string.IsNullOrWhiteSpace(surfaceId) || surfaceId.Length > 256) throw new ArgumentException("Invalid surface id.", nameof(surfaceId));
        if (string.IsNullOrWhiteSpace(contentHash)) throw new ArgumentException("A surface content hash is required.", nameof(contentHash));
        ...
        if (string.IsNullOrWhiteSpace(surfaceSchema)) throw new ArgumentException("A surface schema is required.", nameof(surfaceSchema));
        if (revision < 1) throw new ArgumentOutOfRangeException(nameof(revision));
        _ = NormalizeHash(contentHash);
        if (string.IsNullOrWhiteSpace(correlationId) || correlationId.Length > 256) throw new ArgumentException("Invalid correlation id.", nameof(correlationId));
        if (string.IsNullOrWhiteSpace(causeKind) || causeKind.Length > 64 || string.IsNullOrWhiteSpace(causeId) || causeId.Length > 256)
            throw new ArgumentException("A bounded surface cause is required.");
        if (payload.ValueKind != JsonValueKind.Object) throw new ArgumentException("A surface payload must be an object.", nameof(payload));
        SurfacePayloadPolicy.DemandSafe(payload);
        if (Encoding.UTF8.GetByteCount(payload.GetRawText()) > MaximumSurfacePayloadBytes)
            throw new ArgumentException("The surface payload exceeds the durable delivery bound.", nameof(payload));
        if (requiredClientCapabilities.Count > MaximumCapabilitiesPerSurface || requiredClientCapabilities.Any(static capability =>
            ...
            throw new ArgumentException("Surface capability requirements exceed the delivery bound.", nameof(requiredClientCapabilities));
        if (actions.Count > MaximumActionsPerSurface)
            throw new ArgumentException("Action bindings exceed the delivery bound.", nameof(actions));
        if (protocolVersion != UiProtocol.ProtocolVersion || !string.Equals(surfaceSchema, UiProtocol.SurfaceSchema, StringComparison.Ordinal) ||
            surfaceSchemaVersion != UiProtocol.SurfaceSchemaVersion || actionSchemaVersion != UiProtocol.ActionSchemaVersion)
            throw new ArgumentException("Unsupported durable UI protocol or schema metadata.");
        if (actions.Any(static action => action is null))
            throw new ArgumentException("Action bindings cannot contain null records.", nameof(actions));
        if (actions.Select(static action => action.BindingId).Distinct(StringComparer.Ordinal).Count() != actions.Count)
            throw new ArgumentException("Action binding ids must be unique.", nameof(actions));
        foreach (var action in actions)
        {
            if (action is null || string.IsNullOrWhiteSpace(action.BindingId) || string.IsNullOrWhiteSpace(action.ActionType) ||
                ...
                throw new ArgumentException("A complete action binding is required.", nameof(actions));
        }
        var normalizedHash = NormalizeHash(contentHash);
        var recomputedHash = SurfaceContentHash.Compute(payload, actions);
        if (!CryptographicOperations.FixedTimeEquals(
                Encoding.ASCII.GetBytes(normalizedHash),
                Encoding.ASCII.GetBytes(recomputedHash)))
            throw new ArgumentException("The surface content hash does not match its token-free content.", nameof(contentHash));
    }
    ...
    public static SurfaceAudience Audience(RequestContext context, SurfaceAudienceKind kind) => kind switch
    {
        SurfaceAudienceKind.Principal => new(kind, PrincipalScope.Id(context.Principal)),
        SurfaceAudienceKind.Workspace => new(kind, context.WorkspaceId.Value),
        SurfaceAudienceKind.Public => new(kind, string.Empty),
        _ => throw new ArgumentOutOfRangeException(nameof(kind))
    };

    private static FeedKey Key(RequestContext context, SurfaceAudienceKind kind)
    {
        var audience = Audience(context, kind);
        return new(context.TenantId.Value, context.WorkspaceId.Value, kind, audience.Id,
            kind == SurfaceAudienceKind.Principal ? context.Principal.Kind : null);
    }

    private static AckKey Ack(RequestContext context, SurfaceAudienceKind kind)
    {
        var key = Key(context, kind);
        return new(key, context.Principal.Value, context.Principal.Kind, context.SessionId);
    }

    private static string CursorNonce(RequestContext context, SurfaceAudienceKind kind, long sequence) =>
    ...
    private sealed class FeedState
    {
        ...
        public List<StoredSurfaceRecord> Items { get; } = [];
        public Dictionary<string, StoredSurfaceRecord> Current { get; } = new(StringComparer.Ordinal);
...
public sealed class FilePrivateFeedStore : IPrivateFeedStore
{
    private readonly PrivateFeedStore _inner;

    public FilePrivateFeedStore(string root, byte[]? integrityKey = null) =>
        _inner = new PrivateFeedStore(Path.Combine(root, "v2-feeds", "feed.jsonl"), integrityKey: integrityKey);

    public StoredSurfaceRecord Append(RequestContext context, string surfaceId, int revision, string contentHash, JsonElement payload) =>
        _inner.Append(context, surfaceId, revision, contentHash, payload);
    public FeedPage CatchUp(RequestContext context, long? after, int limit = 50) => _inner.CatchUp(context, after, limit);
    public void RetainFrom(RequestContext context, long minimumSequence) => _inner.RetainFrom(context, minimumSequence);

    public StoredSurfaceRecord Append(RequestContext context, SurfaceAudienceKind audienceKind, string surfaceId, int revision,
        string contentHash, DateTimeOffset createdAt, DateTimeOffset? expiresAt, string correlationId, string causeKind,
        string causeId, IReadOnlyList<string> requiredClientCapabilities, JsonElement payload, IReadOnlyList<StoredActionBinding> actions,
        int protocolVersion = UiProtocol.ProtocolVersion, string surfaceSchema = UiProtocol.SurfaceSchema,
        int surfaceSchemaVersion = UiProtocol.SurfaceSchemaVersion, int actionSchemaVersion = UiProtocol.ActionSchemaVersion) =>
    ...
    public StoredSurfaceRecord EnsureInitial(RequestContext context, SurfaceAudienceKind audienceKind, string surfaceId, Func<long, StoredSurfaceRecord> factory) =>
        _inner.EnsureInitial(context, audienceKind, surfaceId, factory);
    public FeedPage CatchUp(RequestContext context, SurfaceAudienceKind audienceKind, long? after, int limit = 50) =>
        _inner.CatchUp(context, audienceKind, after, limit);
    public ValueTask WaitForChangeAsync(RequestContext context, SurfaceAudienceKind audienceKind, long after, CancellationToken cancellationToken) =>
    ...
    public ValueTask<bool> WaitUntilDeliveredAsync(RequestContext context, SurfaceAudienceKind audienceKind, long sequence, CancellationToken cancellationToken) =>
...
// Legacy registration shape retained for existing callers; live UI uses Issue below with full owner scope.
public sealed record ActionBinding(string BindingId, string TemplateId, int TemplateVersion, string InputSchemaRef, int MaxUses, DateTimeOffset ExpiresAt, string IssuedTokenHash);
public sealed record UiActionUseRecord(string BindingId, string OperationId, string IdempotencyKey, DateTimeOffset UsedAt);
public sealed record ActionSubmission(string OperationId, string IdempotencyKey, JsonElement Input, string ActionType);
public sealed record IssuedAction(string BindingId, string ActionType, string Token, DateTimeOffset ExpiresAt);
...
public sealed class ActionExecutor(IPrivateFeedStore? feed = null)
{
    ...
    private readonly ConcurrentDictionary<string, UiActionUseRecord> _records = new(StringComparer.Ordinal);
    ...
    public void Register(ActionBinding binding)
    {
        ...
        _bindingsByTokenHash[hash] = new IssuedBinding(binding.BindingId, "legacy", binding.InputSchemaRef, null, 0, null, null, null, null, null,
            string.Empty, binding.MaxUses, binding.ExpiresAt, hash, null, null, string.Empty, SurfaceAudienceKind.Principal, string.Empty);
    }

    public IssuedAction Issue(RequestContext recipient, StoredSurfaceRecord surface, StoredActionBinding binding, TimeSpan tokenLifetime)
    {
        ...
        if (surface.Audience.Kind != SurfaceAudienceKind.Principal ||
            !string.Equals(surface.Audience.Id, PrincipalScope.Id(recipient.Principal), StringComparison.Ordinal) ||
        ...
        if (expires <= DateTimeOffset.UtcNow) throw new ActionRejectedException(ActionRejection.Expired);
    ...
    public void NoteCurrentRevision(RequestContext context, SurfaceAudience audience, string surfaceId, int revision) =>
    ...
    public ActionSubmission Use(RequestContext context, string bindingId, string issuedToken, JsonElement input) =>
        Use(context, bindingId, issuedToken, null, 0, input);

    public ActionSubmission Use(RequestContext context, string bindingId, string issuedToken, string? surfaceId, int surfaceRevision, JsonElement input)
    {
        ...
        {
            if (((IssuedBinding)authorization.BindingState).SurfaceId is null) throw new InvalidOperationException("Action binding usage limit exceeded.");
            throw new ActionRejectedException(ActionRejection.Replay);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                if (binding.SurfaceId is null) throw new InvalidOperationException("Action binding usage limit exceeded.");
                throw new ActionRejectedException(ActionRejection.Replay);
            }
        ...
        JsonElement input)
    {
        ...
        if (binding.TokenOwnerKey is { } tokenOwnerKey &&
            ...
            throw new ActionRejectedException(ActionRejection.Forged);
        if (binding.ExpiresAt <= DateTimeOffset.UtcNow) throw new ActionRejectedException(ActionRejection.Expired);
        if (binding.Tenant is not null && !string.Equals(binding.Tenant, context.TenantId.Value, StringComparison.Ordinal))
            throw new ActionRejectedException(ActionRejection.WrongWorkspace);
        ...
        if (binding.Principal is not null && !string.Equals(binding.Principal, context.Principal.Value, StringComparison.Ordinal))
            throw new ActionRejectedException(ActionRejection.WrongOwner);
        ...
        return new Authorization(binding,
            new ActionSubmission(operationId, idempotency, input.Clone(), binding.ActionType))
        {
    ...
    public bool Commit(Authorization authorization, string? durableOperationId = null)
    {
        ...
        if (Interlocked.Exchange(ref authorization.Completed, 1) != 0)
            throw new InvalidOperationException("The action authorization has already completed.");
        try
        {
            var binding = (IssuedBinding)authorization.BindingState;
            ...
            _records[submission.IdempotencyKey] = new UiActionUseRecord(
    ...
    public bool TryGetUse(string idempotencyKey, out UiActionUseRecord? record) => _records.TryGetValue(idempotencyKey, out record);

    public sealed class Authorization
    {
        internal Authorization(object bindingState, ActionSubmission submission)
        {
        ...
        public ActionSubmission Submission { get; }
    ...
    {
        if (string.Equals(schemaRef, WorkspaceSurfaceProducer.InoInputSchema, StringComparison.Ordinal) &&
    ...
    private readonly record struct SurfaceOwnerKey(string Tenant, string Workspace, string Principal, PrincipalKind PrincipalKind,
        SurfaceAudienceKind AudienceKind, string AudienceId, string SurfaceId);
    private readonly record struct BindingOwnerKey(string Tenant, string Workspace, string Principal, PrincipalKind PrincipalKind,
        SurfaceAudienceKind AudienceKind, string AudienceId, string SurfaceId, int Revision, string BindingId);
