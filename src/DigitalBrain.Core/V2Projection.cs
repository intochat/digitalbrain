...
public sealed record V2ProjectionCheckpoint(string Projection, string OwnerId, long CommitSequence, long DirectoryEpoch);
public sealed record V2PoisonRecord(string Projection, string OwnerId, long CommitSequence, string SafeReason, DateTimeOffset QuarantinedAt);

public interface IV2CommitSource
{
...
public interface IV2ProjectionSink
{
    string Name { get; }
    Task ApplyAsync(string ownerId, AggregateCommit commit, CancellationToken cancellationToken = default);
    Task<V2ProjectionCheckpoint?> ReadCheckpointAsync(string ownerId, CancellationToken cancellationToken = default);
    Task SaveCheckpointAsync(V2ProjectionCheckpoint checkpoint, CancellationToken cancellationToken = default);
    Task QuarantineAsync(V2PoisonRecord poison, CancellationToken cancellationToken = default);
}
...
public sealed class V2ProjectionWorker(IV2CommitSource source, IV2ProjectionSink sink)
{
    public async Task<int> RunFullCycleAsync(DirectoryScanCursor cursor, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var owner in owners.OrderBy(x => x.RegistrationSequence).ThenBy(x => x.OwnerId, StringComparer.Ordinal))
        {
            ...
            while (true)
            {
                ...
                try
                {
                    if (!string.Equals(commit.Checksum, V2CommitSeal.Compute(commit.Events), StringComparison.OrdinalIgnoreCase))
                        throw new InvalidDataException("V2 commit checksum mismatch.");
                    await sink.ApplyAsync(owner.OwnerId, commit, cancellationToken);
                    sequence = commit.CommitSequence;
                    await sink.SaveCheckpointAsync(new V2ProjectionCheckpoint(sink.Name, owner.OwnerId, sequence, owner.DirectoryEpoch), cancellationToken);
                    applied++;
                }
                catch (Exception ex) when (ex is InvalidDataException or JsonException)
                {
                    await sink.QuarantineAsync(new V2PoisonRecord(sink.Name, owner.OwnerId, commit.CommitSequence, V2Redaction.SafeSummary(ex.Message), DateTimeOffset.UtcNow), cancellationToken);
                    sequence = commit.CommitSequence;
                }
            }
        }
...
public sealed class InMemoryV2CommitSource : IV2CommitSource
{
    ...
    public void Append(string ownerId, AggregateCommit commit)
    {
        ...
        lock (commits)
        {
            ...
            if (commit.CommitSequence != expected) throw new InvalidOperationException("V2 commit sequence must be contiguous.");
...
public sealed class InMemoryV2ProjectionSink(string name) : IV2ProjectionSink
{
    private readonly ConcurrentDictionary<string, V2ProjectionCheckpoint> _checkpoints = new(StringComparer.Ordinal);
    public string Name { get; } = name;
    public ConcurrentQueue<(string Owner, AggregateCommit Commit)> Applied { get; } = new();
    public ConcurrentQueue<V2PoisonRecord> Poison { get; } = new();
    ...
    public Task<V2ProjectionCheckpoint?> ReadCheckpointAsync(string ownerId, CancellationToken cancellationToken = default) { _checkpoints.TryGetValue(ownerId, out var value); return Task.FromResult(value); }
    public Task SaveCheckpointAsync(V2ProjectionCheckpoint checkpoint, CancellationToken cancellationToken = default) { _checkpoints[checkpoint.OwnerId] = checkpoint; return Task.CompletedTask; }
    public Task QuarantineAsync(V2PoisonRecord poison, CancellationToken cancellationToken = default) { Poison.Enqueue(poison); return Task.CompletedTask; }
}
