...
public sealed class FileProjectionSink(string root, string name) : IProjectionSink
{
    ...
    public Task<ProjectionCheckpoint?> ReadCheckpointAsync(string ownerId, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var path = CheckpointPath(ownerId);
        if (!File.Exists(path)) return Task.FromResult<ProjectionCheckpoint?>(null);
        return Task.FromResult(JsonSerializer.Deserialize<ProjectionCheckpoint>(File.ReadAllText(path), _options));
    }

    public Task SaveCheckpointAsync(ProjectionCheckpoint checkpoint, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    public Task QuarantineAsync(PoisonRecord poison, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
