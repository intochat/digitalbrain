...
public static class ConversationSurfacePayload
{
    ...
    public static JsonElement Build(InoConversationSnapshot conversation)
    {
        ...
        var messages = conversation.Turns
            .TakeLast(MaximumFeedTurns)
            .Select(static turn => new FeedMessage(
                TurnKey(turn),
                turn.Role,
                BoundedFeedText(turn.Text),
                turn.State))
            .ToList();
    ...
    private static JsonElement Serialize(
        ...
        IReadOnlyList<FeedMessage> messages) => JsonSerializer.SerializeToElement(new
        {
            kind = "native",
