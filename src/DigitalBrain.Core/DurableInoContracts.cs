...
/// <summary>
/// Orleans is the only authority for these records. Adapters may carry them but never advance their state.
/// </summary>
...
public sealed record OperationFeedTurn(
    [property: Id(0)] string CommandId,
...
/// <summary>
/// The bounded, UI-safe view of one immutable operation phase. It deliberately has no provider input,
/// OAuth payload, token, credential, or provider response field.
/// </summary>
...
/// <summary>
/// The sole immutable durable projection contract for an operation phase. It is serialized into the
/// conversation outbox and is deliberately sufficient to render that phase without re-reading mutable
/// conversation state.
/// </summary>
    ...
    [property: Id(5)] DateTimeOffset OccurredAt)
{
        ...
        WorkflowReference? workflow = null)
    {
        ...
        var normalizedAction = view.Action is { } action && OAuthCallbackPaths.IsStructurallyValidAction(action)
            ? action
            : null;
    ...
    public static bool TryRead(byte[] payloadUtf8, out OperationOutboxRecord? record)
    {
        ...
        {
            // Rolling deployment compatibility: pre-canonical payloads remain opaque and are never exposed.
            return false;
        }
    }
    ...
    public InoConversationSnapshot ToSnapshot()
    {
        ...
        var view = View!;
        var action = view.Action is { } candidate && OAuthCallbackPaths.IsStructurallyValidAction(candidate)
            ? candidate
            : null;
        return new(
            ConversationId,
    ...
    private bool IsCurrent() =>
        ...
        string.Equals(EventType, PhaseEventType, StringComparison.Ordinal) &&
...
/// <summary>
/// An opaque reference only. It deliberately carries no lifecycle state from an agent or workflow engine.
/// </summary>
[GenerateSerializer, Alias("digitalbrain.runtime.workflow-reference")]
public sealed record WorkflowReference(
    [property: Id(0)] string Runner,
    [property: Id(1)] string WorkflowId,
    [property: Id(2)] string SessionId,
    [property: Id(3)] string? CheckpointId = null);

public sealed record InoAuthorizationResume(
    string Provider,
    string ToolId,
...
/// <summary>
/// A provider-safe OAuth handoff prepared by a typed tool. The opaque flow reference is already bound to the
/// authenticated owner by the OAuth state protector; no token or provider request payload is persisted here.
/// </summary>
...
/// <summary>
/// A validated descriptor, not a model-supplied provider payload. Provider inputs remain in the typed tool
/// boundary and are never written to telemetry or Agent Framework state.
/// </summary>
public sealed record InoToolRequest(
    string ToolId,
    InoToolAccess Access,
    string Scope,
    string SafeSummary);
...
public sealed record InoToolEffectRequest(
    string OperationId,
    string EffectId,
    string ToolId,
    string Scope,
...
/// <summary>
/// Microsoft Agent Framework is confined behind this boundary. It can execute an isolated run but cannot
/// mutate Orleans operation state, write outbox entries, or decide retries.
/// </summary>
