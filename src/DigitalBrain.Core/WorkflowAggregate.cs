...
public sealed class WorkflowAggregate(IAggregateStore store)
{
    ...
    private async Task<V2AggregateSnapshot> ApproveCoreAsync(string aggregateId, string commandId, RequestContext context, ApprovalRecord approval, OutboxRecord firstEffect, CancellationToken cancellationToken)
    {
        ...
        return await CommitAsync(aggregateId, commandId, context, WorkflowState.ApplyQueued, new WorkflowPersistedState(WorkflowState.ApplyQueued, approval, transitions), [firstEffect], cancellationToken);
    }
    ...
    public async Task<V2AggregateSnapshot> AdvanceAsync(string aggregateId, string commandId, RequestContext context, WorkflowState target, string? reason = null, IReadOnlyList<OutboxRecord>? effects = null, CancellationToken cancellationToken = default)
    {
        ...
        return await CommitAsync(aggregateId, commandId, context, target, new WorkflowPersistedState(target, current.Approval, transitions), effects, cancellationToken);
    }
    ...
    private async Task<V2AggregateSnapshot> CommitTerminalAsync(string aggregateId, string commandId, RequestContext context, WorkflowState target, string reason, CancellationToken cancellationToken)
    {
        ...
        return await CommitAsync(aggregateId, commandId, context, target, new WorkflowPersistedState(target, current.Approval, current.Transitions.Append(new WorkflowTransition(current.State, target, DateTimeOffset.UtcNow, reason)).ToArray()), cancellationToken: cancellationToken);
    }

    private async Task<V2AggregateSnapshot> CommitAsync(string aggregateId, string commandId, RequestContext context, WorkflowState target, WorkflowPersistedState? supplied, IReadOnlyList<OutboxRecord>? effects = null, CancellationToken cancellationToken = default)
    {
        var snapshot = await store.ReadAsync(aggregateId, cancellationToken);
        var current = supplied ?? new WorkflowPersistedState(target, null, [new WorkflowTransition(snapshot.CommitSequence == 0 ? WorkflowState.Proposed : target, target, DateTimeOffset.UtcNow)]);
        var state = JsonSerializer.SerializeToDocument(current, JsonOptions).RootElement.Clone();
        var evt = new EventEnvelope($"v2.workflow.{target}", 2, "event-" + Guid.NewGuid().ToString("N"), context.CorrelationId, null, state);
    ...
    private async Task<WorkflowPersistedState> ReadStateAsync(string aggregateId, CancellationToken cancellationToken)
    {
        var state = (await store.ReadAsync(aggregateId, cancellationToken)).State;
        return state.ValueKind == JsonValueKind.Object ? JsonSerializer.Deserialize<WorkflowPersistedState>(state.GetRawText(), JsonOptions) ?? new(WorkflowState.Proposed, null, []) : new(WorkflowState.Proposed, null, []);
    }
}

[GenerateSerializer, Alias("digitalbrain.v2.workflow-persisted-state")]
public sealed record WorkflowPersistedState(
