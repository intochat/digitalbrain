...
public sealed record AdminPolicy(bool Enabled, bool RequireOperatorAssurance, IReadOnlySet<string> AllowedCapabilities);
public sealed record OperatorBootstrapResult(RequestContext Context, string SessionId);
...
public sealed class OperatorBootstrap(SessionManager sessions, AdminPolicy policy)
{
    ...
    public bool TryConsume(string secret, TenantId tenantId, WorkspaceId workspaceId, string principalId, out OperatorBootstrapResult? result)
    {
        result = null;
        ...
        result = new OperatorBootstrapResult(context, sessionId);
        return true;
    }
