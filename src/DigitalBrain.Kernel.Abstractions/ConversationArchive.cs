...
public sealed record ConversationArchiveState(
    [property: Id(0)] int SchemaVersion,
    [property: Id(1)] long Revision,
    [property: Id(2)] ConversationArchiveSegment? Segment)
{
...
public sealed record ConversationArchiveCursor(
    [property: Id(0)] string SegmentId,
    [property: Id(1)] string Digest,
    [property: Id(2)] long BeforeSequence);

[GenerateSerializer, Alias("digitalbrain.runtime.conversation-archive-page")]
public sealed record ConversationArchivePage(
    [property: Id(0)] ConversationTurn[] Turns,
    [property: Id(1)] ConversationArchiveCursor? NextCursor);
...
public static class ConversationArchiveTransitions
{
    ...
    public static ConversationArchiveSegment? PrepareSegment(
        ...
        ConversationState next)
    {
        ...
        if (nextArchive is null ||
            string.Equals(current.Archive?.HeadSegmentId, nextArchive.HeadSegmentId, StringComparison.Ordinal))
            return null;
        var priorThrough = current.Archive?.ThroughSequence ?? 0;
        var removed = current.Turns
            .Where(turn => turn.Sequence > priorThrough && turn.Sequence <= nextArchive.ThroughSequence)
            .ToArray();
        ...
        Func<string, Task<ConversationArchiveSegment?>> readSegment)
    {
        ...
        while (newestFirst.Count < maximumTurns && nextCursor is not null)
        {
            ...
            var candidates = segment.Turns
                .Where(turn => turn.Sequence < current.BeforeSequence)
            ...
            nextCursor = segment.PreviousSegmentId is null
                ? null
                : new ConversationArchiveCursor(
                    segment.PreviousSegmentId,
                    segment.PreviousDigest!,
    ...
    public static void ValidateDescriptor(ConversationArchiveDescriptor? archive, ConversationTurn[] inlineTurns)
    {
        ...
        if (archive.ArchivedTurnCount < 1 || archive.ThroughSequence != archive.ArchivedTurnCount ||
            archive.FirstTurnAt == default || archive.LastTurnAt == default ||
    ...
    {
        if (!RuntimeStateKeys.IsScopeHash(segment.SegmentId) ||
            !RuntimeStateKeys.IsScopeHash(segment.ConversationScopeHash) ||
        ...
        var expectedId = RuntimeStateKeys.ConversationArchiveSegment(
            segment.ConversationScopeHash,
            segment.PreviousSegmentId,
            segment.ThroughSequence,
            expectedDigest);
    ...
    private static string Digest(string? previousDigest, ConversationTurn[] turns) =>
        Convert.ToHexStringLower(SHA256.HashData(JsonSerializer.SerializeToUtf8Bytes(
            new ArchiveDigestPayload(previousDigest, turns), DigestJson)));

    private static void ValidateCursor(ConversationArchiveCursor cursor)
    {
        if (!RuntimeStateKeys.IsScopeHash(cursor.SegmentId) || !RuntimeStateKeys.IsScopeHash(cursor.Digest) ||
