...
public sealed record InoEffectPlanCompletion(
...
public static class InoEffectPlanTransitions
{
    ...
    public static InoEffectPlanState Complete(
    ...
    {
        if (!RuntimeStateKeys.IsScopeHash(plan.PlanId) || !RuntimeStateKeys.IsScopeHash(plan.ActorScope) ||
            !IsBounded(plan.OperationId, 128) || !IsToolId(plan.ToolId) ||
...
public static class InoMutationGrants
{
    public static string? RequiredForTool(string? toolId) => toolId switch
    {
        GmailTools.Send => "gmail.send",
        SalesforceTools.UpdateRecord => "salesforce.write",
        _ => null
    };

    public static void Demand(string? toolId, IReadOnlySet<string> grants)
    {
        var requiredGrant = RequiredForTool(toolId);
        if (requiredGrant is not null && !grants.Contains(requiredGrant))
