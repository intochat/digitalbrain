...
// The commit-fault seam (§8): wraps the volatile journal storage so a test can make ONE
// neuron's next WriteStateAsync fail — the poison-and-reload physics is provable without
// touching any Core internals. Faults arm per neuron journal, optionally after a countdown
// of allowed commits, optionally sticky until disarmed; consuming a fault completes the
// registration's Consumed task (the test's deterministic sync point). Unconsumed faults
// are leaked test intent and fail the arming test at dispose.
