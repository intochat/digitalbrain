...
// One real cluster per composition (§8): a single-silo InProcessTestCluster — the volatile
// journal provider lives OUTSIDE the silo in this fixture, so state survives deactivation
// and silo restarts, while a second silo would lose reactivated-elsewhere neurons (the
// proven trap). Boot is lazy and once; every test leasing this composition shares the
// brain, the recording storage and the controllable clock.
...
#pragma warning restore CA2213
    ...
    // The DeactivateOnIdle-equivalent a test can drive from outside: collect every idle
    // activation and wait until the named neurons are actually gone — the next touch is a
    // fresh activation that holds only what the one-batch commit made durable.
