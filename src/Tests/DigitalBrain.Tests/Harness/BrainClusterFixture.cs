...
public sealed class BrainClusterFixture : IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        builder.ConfigureSilo((options, silo) => ConfigureSilo(silo, modules));
        builder.ConfigureClient(client => ConfigureClient(client, modules));

        _cluster = builder.Build();
        await _cluster.DeployAsync();
    }

    public async Task RestartSilosAsync()
    {
        var cluster = Cluster;
        // One silo at a time so the client keeps a live gateway between restarts.
        foreach (var silo in cluster.GetActiveSilos().ToArray())
        {
    ...
    private static ModuleAssemblies CreateModules()
        => new(
            [
    ...
    private void ConfigureSilo(ISiloBuilder silo, ModuleAssemblies modules)
    {
    ...
    private static void ConfigureClient(IClientBuilder client, ModuleAssemblies modules)
    {
