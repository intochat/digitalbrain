...
internal sealed class WorkspaceWarmup(
    SolutionWorkspace workspace,
    SolutionFileWatcher watcher,
    IGrainFactory grains,
    IOptions<RoslynModuleOptions> options,
    ILogger<WorkspaceWarmup> logger) : IHostedService
{
    ...
    // The neuron is the durable record of "which solution is open"; the load itself already runs in the
    // service, so this open is a no-op for the service and records the path, generation and map on the neuron.
    private async Task RecordOpenAsync(string fullPath, string key)
    {
        ...
        var neuron = grains.GetGrain<IRoslyn>(key);
        for (var attempt = 0; attempt < 30; attempt++)
        {
            try
            {
                await neuron.Open(new OpenWorkspace(fullPath)).ConfigureAwait(false);
                return;
            }
