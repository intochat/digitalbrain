...
internal sealed class GmailMcpProvider : IGmailProvider
{
        ...
        string accessToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            return await McpHttpSession.RunAsync(GmailModule.GmailMcpEndpoint, accessToken, async (client, token) =>
            {
    ...
    public async Task<string> ReadToolSchemaHashAsync(string tool, string accessToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var catalog = await McpHttpSession.ReadCatalogAsync(GmailModule.GmailMcpEndpoint, accessToken, cancellationToken).ConfigureAwait(false);
            ValidateCatalog(catalog);
