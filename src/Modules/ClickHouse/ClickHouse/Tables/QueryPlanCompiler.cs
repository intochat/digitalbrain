...
// Predicates mirror ClickHouseTablePolicy: text compares case-insensitively, neq keeps nulls,
// ordered comparisons drop them, and sorting puts nulls first ascending and last descending.
internal static partial class QueryPlanCompiler
{
    public static CompiledQuery Compile(QueryPlan plan)
    {
        ...
        for (var index = 0; index < plan.Filters.Count; index++)
        {
            ...
            {
                throw new ClickHouseTableValidationException($"Filter column '{filter.ColumnId}' does not exist.");
            }

            parameters[$"c{index}"] = column.Name;
            if (!ClickHouseTablePolicy.TryElement(filter.Value, out var value))
            {
                throw new ClickHouseTableValidationException($"Filter value for '{filter.ColumnId}' is not valid JSON.");
            }

            predicates.Add(Predicate(column.TableType, filter, value, $"{{c{index}:Identifier}}", $"p{index}", parameters));
        }
    ...
    private static string Predicate(string tableType, ClickHouseTableFilter filter, JsonElement value, string column, string parameter, Dictionary<string, object> parameters)
    {
        ...
        var operand = Operand(tableType, value, parameter, parameters);
        if (tableType == ClickHouseTypeMap.Text)
        {
            ...
            return filter.Operator switch
            {
                ...
                _ => throw new ClickHouseTableValidationException($"Operator '{filter.Operator}' is not supported for text."),
            };
        }
        ...
        return filter.Operator switch
        {
            ...
            "lt" => $"{column} < {operand}",
            "lte" => $"{column} <= {operand}",
            _ => throw new ClickHouseTableValidationException($"Operator '{filter.Operator}' is not supported for {tableType}."),
        };
    }
    ...
    private static string Operand(string tableType, JsonElement value, string parameter, Dictionary<string, object> parameters)
    {
        switch (tableType)
        {
            ...
            case ClickHouseTypeMap.Date:
                parameters[parameter] = value.GetString() ?? throw new ClickHouseTableValidationException("Date filters need a yyyy-MM-dd value.");
    ...
    private static string OrderBy(QueryPlan plan, Dictionary<string, ClickHouseColumn> columns, Dictionary<string, object> parameters)
    {
        ...
        if (plan.Sort is { } sort)
        {
            ...
            {
                throw new ClickHouseTableValidationException($"Sort column '{sort.ColumnId}' does not exist.");
            }
