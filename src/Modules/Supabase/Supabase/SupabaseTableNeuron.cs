...
// An ITable whose rows live in Supabase. The saved view (columns, filters, sort, visible
...
internal sealed class SupabaseTableNeuron(
    ...
    [PersistentState("state", DigitalBrainNames.DefaultGrainStorage)] IPersistentState<SnapshotEnvelope<SupabaseTableState>> state)
    : Neuron<SupabaseTableState>(runtime, state), ISupabaseTable
{
    ...
    public Task<Accepted<string>> CreateFromQuery(CreateQueryTableCommand command, CancellationToken cancellationToken = default)
        => ExecuteCommandAsync(Descriptor("create-query"), command, SupabaseJson.Default.CreateQueryTableCommand, SupabaseJson.Default.AcceptedString, arguments =>
        {
            ...
            try
            {
                SupabaseQueryGuard.Validate(arguments.Table.Sql);
            }
    ...
    {
        var current = State ?? new SupabaseTableState(null, null, [], []);
        var next = current;
        TableOperationResult result;
        switch (delivery.Signal.Type)
        {
            case SupabaseSignals.QueryTableCreating:
                var create = Body(delivery, SupabaseJson.Default.CreateQueryTableCommand);
                ...
                catch (SupabaseUnavailableException error)
                {
                    ...
                    result = new(create.Id, "invalid", error.Message + " Try again once Supabase is reachable.");
                }
                ...
                break;
            case SupabaseSignals.QueryTableUpdating:
    ...
    private TableSnapshot View(string title, IReadOnlyList<SupabaseColumn> columns)
    {
