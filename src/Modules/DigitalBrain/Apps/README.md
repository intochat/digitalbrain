...
- `Install`, `Configure`, `Upgrade` deploy the revision's verified artifact through `IBehaviorProgram` with `Behavior__App` and `Behavior__Settings__{name}` configuration. Configuring declared settings customizes a package without forking it.
