...
public static partial class BehaviorValidator
{
    ...
    public static BehaviorValidation Validate(BehaviorDefinition definition)
    {
        ...
        List<string> errors = [];
        if (string.IsNullOrWhiteSpace(definition.Id) || !BehaviorId().IsMatch(definition.Id))
        {
            errors.Add("Behavior id must be 1–48 letters, digits, underscores, or hyphens, starting with a letter or digit.");
        }
        if (string.IsNullOrWhiteSpace(definition.Name) || definition.Name.Length > 200)
        {
            errors.Add("Behavior name must contain 1–200 characters.");
        }
        if (string.IsNullOrWhiteSpace(definition.Trigger) || !SignalType().IsMatch(definition.Trigger))
        {
            errors.Add("Behavior trigger must be a signal type of 1–64 letters, such as LeadReceived.");
        }
        ...
        {
            errors.Add("Behavior description must contain at most 4,000 characters.");
        }
        ...
        var nodes = new Dictionary<string, BehaviorNode>(StringComparer.Ordinal);
        var configBytes = 0;
        ...
        {
            errors.Add("Behavior node configuration exceeds 64 KB. Store large data externally and pass a reference.");
        }
        ...
        {
            errors.Add("Behavior synapses contain a cycle. Use a new trigger for a later reaction.");
        }

        return new(errors.Count == 0, errors, order);
    }

    private static void ValidateConfig(BehaviorNode node, List<string> errors)
    {
        ...
        switch (node.Kind?.ToLowerInvariant())
        {
            ...
            case "map":
                ...
                break;
            case "output":
                if (BehaviorBuiltins.TryProperty(config, out var template, "template", "value"))
                {
            ...
            case "aggregate":
                ...
                if (BehaviorBuiltins.TryProperty(config, out var operation, "operation")
            ...
            case "call":
                ...
                if (BehaviorBuiltins.TryProperty(config, out var arguments, "arguments"))
                {
            ...
            case "agent":
                ...
                if (BehaviorBuiltins.TryProperty(config, out var agentMode, "mode") && agentMode.ValueKind == JsonValueKind.String)
                {
                ...
                if (BehaviorBuiltins.TryProperty(config, out var mode, "mode") && mode.ValueKind == JsonValueKind.String && mode.GetString() == "web")
                {
                    ...
                    if (BehaviorBuiltins.TryProperty(config, out var webOperation, "operation"))
                    {
    ...
    private static void RequireTemplate(BehaviorNode node, List<string> errors, params string[] names)
    {
        if (!BehaviorBuiltins.TryProperty(node.Config, out var template, names))
        {
    ...
    private static void ValidateTemplate(string id, JsonElement template, List<string> errors)
    {
        try
        {
            _ = BehaviorExpressions.Resolve(template, BehaviorExpressions.Null, BehaviorExpressions.Null,
    ...
    private static void ValidatePredicate(string id, JsonElement predicate, List<string> errors, int depth)
    {
        ...
        {
            if (BehaviorBuiltins.TryProperty(predicate, out var children, group))
            {
        ...
        if (BehaviorBuiltins.TryProperty(predicate, out var not, "not"))
        {
        ...
        var operation = "eq";
        if (BehaviorBuiltins.TryProperty(predicate, out var configuredOperator, "operator"))
        {
        ...
        if (BehaviorBuiltins.TryProperty(predicate, out var expected, "value"))
        {
    ...
    {
        if (BehaviorBuiltins.TryProperty(config, out var value, property) && value.ValueKind != JsonValueKind.String)
        {
    ...
    {
        if (!BehaviorBuiltins.TryProperty(config, out var value, property)
    ...
    private static partial Regex BehaviorId();
