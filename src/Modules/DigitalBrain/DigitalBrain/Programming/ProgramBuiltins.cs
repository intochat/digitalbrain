...
public sealed record ProgramNodeResult(JsonElement Output, bool Stop = false);

public static class ProgramBuiltins
{
    public static ProgramNodeResult Execute(ProgramNode node, JsonElement input, JsonElement value,
        IReadOnlyDictionary<string, JsonElement> outputs)
    {
        ...
        {
            "input" => new(ProgramExpressions.Clone(input)),
            "template" => new(ProgramExpressions.Resolve(Required(node.Config, "template", "text"), input, value, outputs)),
            "filter" => new(ProgramExpressions.Clone(value), !Matches(node.Config, input, value, outputs)),
            ...
            "output" => new(TryProperty(node.Config, out var template, "template", "value")
                ? ProgramExpressions.Resolve(template, input, value, outputs)
                : ProgramExpressions.Clone(value)),
    ...
    private static JsonElement Select(JsonElement config, JsonElement input, JsonElement value,
        IReadOnlyDictionary<string, JsonElement> outputs)
        => ProgramExpressions.ResolvePath(String(config, "path"), input, value, outputs);
        ...
        IReadOnlyDictionary<string, JsonElement> outputs)
    {
        ...
        {
            return ProgramExpressions.Resolve(template, input, source, outputs);
        }
        ...
        using (var writer = new Utf8JsonWriter(buffer))
        {
            ...
            {
                ProgramExpressions.Resolve(template, input, item, outputs).WriteTo(writer);
                ProgramExpressions.RequireOutputRoom(writer);
            }
        ...
        IReadOnlyDictionary<string, JsonElement> outputs)
    {
        ...
        if (operation != "count")
        {
            foreach (var item in source.EnumerateArray())
            {
                var number = ProgramExpressions.ReadPath(item, field);
        ...
        IReadOnlyDictionary<string, JsonElement> outputs, int depth = 0)
    {
        ...
        var expected = TryProperty(predicate, out var requested, "value")
            ? ProgramExpressions.Resolve(requested, input, value, outputs)
            : ProgramExpressions.Null;
