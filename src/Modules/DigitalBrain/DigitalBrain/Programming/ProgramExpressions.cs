...
public static partial class ProgramExpressions
{
        ...
        IReadOnlyDictionary<string, JsonElement> outputs, int depth)
    {
        if (depth > 32)
        {
            throw new ArgumentException("Program templates may nest at most 32 levels.", nameof(template));
        }
    ...
    internal static void RequireOutputRoom(Utf8JsonWriter writer)
    {
        ...
        {
            throw new ArgumentException("Program output exceeds the 64 KB signal limit. Return smaller data or an external reference.", nameof(writer));
        }
    }
