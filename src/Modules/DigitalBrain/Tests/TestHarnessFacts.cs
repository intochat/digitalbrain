...
public sealed class TestHarnessFacts
{
    ...
    public async Task ReadinessBelongsToEachBehaviorAndTheTriggerRunsOnce()
    {
        ...
        var source = host.Brain.Get<ITestEmitter>("behavior");
        ...
        async Task Observe(IDigitalBrain brain, TaskCompletionSource<int> result, CancellationToken token)
        {
        ...
        await using var first = host.RunBehavior((brain, token) => Observe(brain, a, token), ct);
        await using var second = host.RunBehavior((brain, token) => Observe(brain, b, token), ct);
    ...
    public async Task EarlyBehaviorFailureIsNotReportedAsReadiness()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var host = await BrainTestHost.StartAsync(cancellationToken: ct);
        var run = host.RunBehavior((_, _) => Task.FromException(new IOException("behavior failed")), ct);
        await Assert.ThrowsAsync<IOException>(() => run.WaitForSubscriptionAsync<Number>(host.Brain.Get<ITestEmitter>("x"), ct));
    ...
    public async Task ProbeObservesOneTypedFact()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var host = await BrainTestHost.StartAsync(cancellationToken: ct);
        var source = host.Brain.Get<ITestEmitter>("probe");
        await using var probe = await host.ObserveAsync<Number>(source, ct);
        await source.Emit(4);
    ...
    public async Task ProbeTimeoutIsBoundedAndNoncooperativeBehaviorCannotHangTeardown()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var host = await BrainTestHost.StartAsync(cancellationToken: ct);
        await using var probe = await host.ObserveAsync<Number>(host.Brain.Get<ITestEmitter>("silent"), ct);
        var never = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var run = host.RunBehavior((_, _) => never.Task, ct);
        try
        {
