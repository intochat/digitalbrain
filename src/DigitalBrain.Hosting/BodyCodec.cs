...
internal sealed class BodyCodec : ISynapseCodec
{
    ...
    public object? Decode(JsonElement element, Type type)
        => JsonSerializer.Deserialize(element, type, Options);

    public Synapse? DecodeFact(string kind, JsonElement body)
    ...
    private void ProbeMember(Type owner, string member, Type memberType, HashSet<Type> visited)
    {
        ...
        if (memberType.IsArray)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{Catalog.Describe(owner)}.{member} is a multidimensional array; journals support one-dimensional data only.");
            }
