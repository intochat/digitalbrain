...
internal sealed class SynapseSerialization : ISynapseSerialization
{
    ...
    internal SynapseSerialization(CompositionCatalog catalog)
    {
    ...
    public Synapse? DeserializeForDispatch(string synapseKind, JsonElement serialization)
        => catalog.TryGetSynapseType(synapseKind, out var synapseType)
            ? (Synapse?)JsonSerializer.Deserialize(serialization, synapseType, Options)
            : null;

    internal void ValidateVocabulary()
    {
        var visited = new HashSet<Type>();
        foreach (var synapseType in catalog.SynapseTypes)
        {
            ProbeShape(synapseType, visited);
        }
    }
    ...
    internal void ValidateState(Type stateType)
    {
        ...
        if (!stateType.GetConstructors().Any(static constructor => constructor.GetParameters().All(static parameter => parameter.HasDefaultValue)))
        {
            throw new InvalidOperationException(
                $"{CompositionCatalog.Describe(stateType)} has no constructor callable without arguments; "
                + "Hosting must be able to materialize an empty turn state.");
        }
        ...
        foreach (var member in stateType.GetMembers(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{CompositionCatalog.Describe(stateType)}.{member.Name} is required; a required member bricks "
                    + "materialization of empty turn state — give it a default instead.");
            }
        }
    ...
    private void ProbeMember(Type owner, string member, Type memberType, HashSet<Type> visited)
    {
        ...
        {
            throw new InvalidOperationException(
                $"{CompositionCatalog.Describe(owner)}.{member} is {CompositionCatalog.Describe(memberType)}, "
                + "which cannot travel through synapse serialization; journals hold data, never runtime artifacts.");
        }
        ...
        if (memberType.IsArray)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{CompositionCatalog.Describe(owner)}.{member} is a multidimensional array; journals support one-dimensional data only.");
            }
        ...
        if (typeof(System.Collections.IEnumerable).IsAssignableFrom(memberType))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{CompositionCatalog.Describe(owner)}.{member} is the untyped collection {CompositionCatalog.Describe(memberType)}; "
                    + "serialization cannot know what to materialize — use a generic collection.");
            }
        ...
        if (memberType.IsInterface || memberType.IsAbstract)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{CompositionCatalog.Describe(owner)}.{member} is abstract {CompositionCatalog.Describe(memberType)} with no "
                    + "[JsonDerivedType] map; it would serialize its declared shape and never materialize.");
            }
    ...
    private static InvalidOperationException Unresolvable(Type owner, Type type, string? member, Exception failure)
        => new(member is null
            ? $"{CompositionCatalog.Describe(owner)} cannot travel through synapse serialization: {failure.Message}"
            : $"{CompositionCatalog.Describe(owner)}.{member} ({CompositionCatalog.Describe(type)}) cannot travel through synapse serialization: {failure.Message}",
            failure);

    private sealed class SynapseConverterFactory(CompositionCatalog catalog) : JsonConverterFactory
    {
        ...
        public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
            => (JsonConverter)Activator.CreateInstance(
                typeof(SynapseConverter<>).MakeGenericType(typeToConvert),
                catalog)!;
    }

    private sealed class SynapseConverter<TSynapse>(CompositionCatalog catalog) : JsonConverter<TSynapse>
        where TSynapse : Synapse
    {
        public override TSynapse? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
                ...
                || kindProperty.GetString() is not { } kind)
            {
                throw new JsonException(
                    $"A polymorphic synapse position materializes from '{KindProperty}'/'{BodyProperty}', "
                    + $"but no '{KindProperty}' is present.");
            }

            if (!catalog.TryGetSynapseType(kind, out var synapseType))
            {
            ...
            return JsonSerializer.Deserialize(body, synapseType, options) as TSynapse
                ?? throw new JsonException(
                    $"Synapse kind '{kind}' materialized as {CompositionCatalog.Describe(synapseType)}, "
        ...
        public override void Write(Utf8JsonWriter writer, TSynapse value, JsonSerializerOptions options)
        {
            ...
            writer.WriteString(KindProperty, catalog.KindOfSynapse(value.GetType()));
