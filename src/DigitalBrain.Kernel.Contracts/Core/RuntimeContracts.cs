...
public sealed class SessionTokenService
{
    ...
    public string Issue(RequestContext context, TimeSpan lifetime, string audience = SessionAudiences.Mcp, long sessionVersion = 1)
    {
    ...
    public bool TryValidateActionCapability(string token, RequestContext context, string bindingId, string surfaceId, int surfaceRevision, string bindingTokenHash)
    {
        ...
        if (claims is null || claims.Version != 1 || !string.Equals(claims.ScopeId, RequestScope.Id(context), StringComparison.Ordinal) ||
    ...
    private bool TryValidateCore(string token, string? expectedAudience, out RequestContext context, out DateTimeOffset expiresAt, out long sessionVersion)
    {
        ...
        var grants = (claims.Grants ?? []).Where(static grant => !string.IsNullOrWhiteSpace(grant))
            .ToHashSet(StringComparer.Ordinal);
        context = new RequestContext(new(claims.OwnerId), new(claims.ActorId), new(claims.SessionId), claims.Assurance, Guid.NewGuid().ToString("N"), null, grants);
        expiresAt = expiry;
