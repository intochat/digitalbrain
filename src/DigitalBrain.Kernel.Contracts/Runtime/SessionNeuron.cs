...
public interface ISessionNeuron : IGrainWithStringKey
{
    ...
    Task<SessionRotation> RotateRefreshAsync(long expectedRevision, string presentedRefreshHash, string replacementRefreshHash, DateTimeOffset replacementExpiresAt, DateTimeOffset now);
...
public static class SessionTransitions
{
        ...
        DateTimeOffset now)
    {
        ...
        var replayEntry = new SessionRefreshReplay(presentedRefreshHash.ToLowerInvariant(), replacementRefreshHash.ToLowerInvariant(), now);
        var next = state with
        {
            Revision = checked(state.Revision + 1),
            ...
            RefreshReplay = state.RefreshReplay.Append(replayEntry).OrderBy(entry => entry.ConsumedAt)
    ...
    public static SessionState Revoke(SessionState state, long expectedRevision, DateTimeOffset revokedAt)
    {
        ...
        var next = state with { Revision = checked(state.Revision + 1), SessionVersion = checked(state.SessionVersion + 1), RevokedAt = revokedAt };
        Validate(next);
        return next;
    }
