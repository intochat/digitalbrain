...
public static class ConversationTransitions
{
        ...
        ConversationOutboxEntry? runningOutbox = null)
    {
        ...
        if (operation.Status == ConversationOperationStatus.Running && operation.LeaseExpiresAt is { } activeLease && activeLease > now &&
            string.Equals(operation.LeaseOwner, leaseOwner, StringComparison.Ordinal))
            return new(state, operation, true, Acquired: false);
        ...
        ConversationLeaseFence? leaseFence = null)
    {
        ...
        var priorTurn = FindTurn(state, operationId, ConversationTurnKind.Authorization, invocation.AuthorizationAttemptId);
        ...
        if (operation.Status == ConversationOperationStatus.AwaitingAuthorization && SameInvocation(operation.SuspendedInvocation, invocation))
        {
            if ((priorTurn is null || string.Equals(priorTurn.Text, assistantText, StringComparison.Ordinal)) && priorOutbox is not null && SameOutbox(priorOutbox, feedOutbox)) return state;
        ...
        var next = AppendOutbox(AppendTurnRecord(state, operationId, ConversationTurnKind.Authorization, invocation.AuthorizationAttemptId, assistantText, now) with
        {
            Revision = checked(state.Revision + 1),
        ...
        ConversationLeaseFence? leaseFence = null)
    {
        ...
        var priorOutbox = state.Outbox.FirstOrDefault(entry =>
            string.Equals(entry.OutboxId, feedOutbox.OutboxId, StringComparison.Ordinal));
        if (operation.Status == ConversationOperationStatus.AwaitingApproval && operation.Approval == approval && operation.Effect == effect)
        {
            if ((priorTurn is null || string.Equals(priorTurn.Text, assistantText, StringComparison.Ordinal)) && priorOutbox is not null && SameOutbox(priorOutbox, feedOutbox)) return state;
        ...
        var next = AppendOutbox(AppendTurnRecord(state, operationId, ConversationTurnKind.Approval, approval.ApprovalId, assistantText, now) with
        {
            Revision = checked(state.Revision + 1),
            Lifecycle = ConversationLifecycle.Suspended,
        ...
        DateTimeOffset now)
    {
        ...
        if (operation.Status != ConversationOperationStatus.AwaitingApproval || approval.State != "requested" || effect.State != "awaiting-approval")
        ...
        var decidedEffect = effect with { State = effectState, Version = checked(effect.Version + 1) };
        ...
        var next = AppendOutbox(AppendTurnRecord(state, operationId, ConversationTurnKind.Approval, decisionId, assistantText, now) with
        {
            Revision = checked(state.Revision + 1),
        ...
        FeatureDraftReference? proposal = null)
    {
        ...
        var next = AppendOutbox(AppendTurnRecord(state, operationId, ConversationTurnKind.Assistant, operationId, assistantText, now) with
        {
            Revision = checked(state.Revision + 1),
            Lifecycle = ConversationLifecycle.Active,
    ...
    public static ConversationState RemoveLegacyAuthorizationPayloads(ConversationState state)
    {
        ...
        {
            if (operation.SuspendedInvocation is not { } invocation || (invocation.InputUtf8.Length == 0 && invocation.Workflow is null))
                return operation;
            changed = true;
    ...
    public static ConversationState MigrateLegacyOutboxSequences(ConversationState state)
    {
        ...
        var sequence = 0L;
        var outbox = state.Outbox.OrderBy(entry => entry.CreatedAt)
    ...
    public static ConversationState MigrateLegacyAcceptedCommands(ConversationState state)
    {
        ...
        var identity = state.Identity ?? throw new RuntimeStateIntegrityException("legacy accepted-command migration requires a conversation identity");
    ...
    public static void Validate(ConversationState state)
    {
        ...
        var actorScope = state.Identity is { } identity ? RequestScope.Id(identity.OwnerId, identity.ActorId) : null;
        foreach (var operation in state.Operations)
        {
            ValidateOperation(operation);
            if (operation.Approval?.DecidedBy is { } decidedBy && !string.Equals(decidedBy, actorScope, StringComparison.Ordinal))
    ...
    private static ConversationState AppendOutbox(ConversationState state, ConversationOutboxEntry entry)
    {
        ...
        return state with
        {
            Outbox = state.Outbox.Append(entry with { PayloadUtf8 = entry.PayloadUtf8.ToArray(), Sequence = sequence }).ToArray(),
    ...
    private static ConversationState Compact(ConversationState state)
    {
        ...
        return state with
        {
            ...
            Outbox = pendingOutbox.Concat(dispatchedOutbox).OrderBy(entry => entry.Sequence == 0 ? 0 : 1)
    ...
    private static void ValidateApprovalLifecycle(ConversationOperation operation, ApprovalRecord approval, EffectRecord effect)
    {
        var valid = operation.Status switch
        {
            ...
            ConversationOperationStatus.Failed =>
                approval.State == "rejected" && effect.State == "rejected" || approval.State == "approved" && effect.State == "failed",
            _ => false
        };
        if (!valid)
