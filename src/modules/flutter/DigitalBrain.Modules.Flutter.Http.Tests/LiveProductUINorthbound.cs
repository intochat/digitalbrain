    ...
    public async Task PostOpenSceneAndSseProjectsSceneOpened()
    {
        ...
        using (var health = await http.GetAsync(new Uri(UiHttpContract.HealthPath, UriKind.Relative), cancellationToken))
        {
            Assert.True(
                ...
                $"Product {UIFixture.DefaultUIResourceName} {UiHttpContract.HealthPath} not OK at {baseAddress}. Start: aspire start --project os/DigitalBrain.OS.AppHost. Status={(int)health.StatusCode}. Override with {UIFixture.UIBaseEnvironmentVariable}.");
        }
