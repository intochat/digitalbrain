...
public sealed class AuthorizationProjection(UIFixture fixture)
{
    ...
    [Fact(DisplayName = "UiHttpContract names the authorization events path and event")]
        ...
        CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"{UiHttpContract.AuthorizationEventsPath}?{UiHttpContract.AfterSequenceQuery}={afterSequence}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (!linked.IsCancellationRequested)
        {
            ...
            if (line.Length == 0
                && string.Equals(eventName, UiHttpContract.AuthorizationEvent, StringComparison.Ordinal)
