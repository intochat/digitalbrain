...
internal sealed class SemanticIntentValidator
{
    ...
    // Shared with McpIntegrationPlanner.PlanAsync -- both need "which single tool does this semantic
    // provider/operation pair resolve to."
    internal static string? ExpectedTool(SemanticIntentProposal proposal) => proposal.Provider switch
    {
    ...
    // Shared with ToolResultComposer/ProviderInvocationPlanner -- both walk a provider result tree
    // looking for record ids to surface as grounding.
    internal IEnumerable<string> SalesforceRecordIds(JsonElement value)
    {
        if (value.ValueKind == JsonValueKind.String)
        {
            if (IsValidSalesforceRecordId(value.GetString()))
            {
        ...
        if (value.TryGetProperty("RecordId", out var recordId) && recordId.ValueKind == JsonValueKind.String &&
            IsValidSalesforceRecordId(recordId.GetString()))
