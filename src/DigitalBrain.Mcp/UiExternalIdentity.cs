...
public sealed record UiExternalIdentityOptions(
    bool Enabled,
    string Issuer,
    string Audience,
    string SubjectClaim,
    ...
    bool RequireHttpsMetadata)
{
    ...
    public static UiExternalIdentityOptions FromConfiguration(
        ...
        RuntimeProfile profile)
    {
        ...
        if (!Uri.TryCreate(issuer, UriKind.Absolute, out var issuerUri) || issuerUri.Host.Length == 0 ||
    ...
    private static UiExternalIdentityOptions Disabled() => new(
        false,
        string.Empty,
        string.Empty,
        "sub",
    ...
    private static string ClaimName(string? configured, string fallback, string name)
    {
        ...
        return ValidBounded(value, 128)
            ? value
            : throw new InvalidOperationException($"{SectionPath}:{name} is invalid.");
    }
    ...
    private static string[] ReadValues(IConfigurationSection section, string? scalar) =>
        section.GetChildren().Select(static child => child.Value)
            ...
            .Concat((scalar ?? string.Empty).Split(
                [',', ' ', ';'],
                StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
            .Distinct(StringComparer.Ordinal)
            .ToArray();
...
public sealed record UiExternalAuthenticationResult(
    UiExternalAuthenticationStatus Status,
    RuntimeRequestContext? Context = null);
...
public sealed class UiExternalIdentityAuthenticator(UiExternalIdentityOptions options)
{
    ...
    public async Task<UiExternalAuthenticationResult> AuthenticateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var result = await httpContext.AuthenticateAsync(UiExternalIdentityOptions.AuthenticationScheme)
                .ConfigureAwait(false);
            if (!result.Succeeded || result.Principal is null ||
                !options.TryMapPrincipal(result.Principal, out var mapped))
                return new(UiExternalAuthenticationStatus.Rejected);
