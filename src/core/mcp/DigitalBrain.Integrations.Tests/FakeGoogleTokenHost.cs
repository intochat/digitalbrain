...
internal sealed class FakeGoogleTokenHost : IAsyncDisposable
{
    ...
    public object? RefreshError { get; set; }
    ...
    public static async Task<FakeGoogleTokenHost> StartAsync()
    {
        ...
        app.MapPost("/token", async (HttpRequest request) =>
        {
            ...
            {
                host.RefreshCount++;
                if (host.RefreshStatusCode != HttpStatusCode.OK)
                {
                    return Results.Json(
                        host.RefreshError ?? host.RefreshResponse ?? new { error = "invalid_grant" },
                        statusCode: (int)host.RefreshStatusCode);
                }
