...
internal sealed class IntoChatProgramExecutor(IServiceProvider services, INeuronInvoker invoker,
    ProgramCodeRunner codeRunner, ProgramLiveEvents events) : IProgramNodeExecutor
{
    ...
    private async Task<JsonElement> ExecuteCoreAsync(ProgramExecutionContext context, CancellationToken cancellationToken)
    {
        ...
        var config = ProgramExpressions.Resolve(context.Node.Config, context.Input, context.Value, context.Outputs);
    ...
    private async Task<JsonElement> CallAsync(ProgramExecutionContext context, JsonElement config, CancellationToken cancellationToken)
    {
        ...
        if (!descriptor.IsReadOnly)
        {
            ...
            var hash = SHA256.HashData(Encoding.UTF8.GetBytes($"{context.ProgramId}/{context.RunId}/{context.Version}/{context.Node.Id}"));
    ...
    private async Task<JsonElement> ConverseAsync(ProgramExecutionContext context, JsonElement config, CancellationToken cancellationToken)
    {
        ...
        {
            messages.AddRange(ProgramConversationHistory.Read(inputMessages));
        }
        ...
        ProgramConversationHistory.Trim(messages, historyBudget);
