...
internal sealed class ProgramConversationEndpoint(ProgramService programs, IGrainFactory grains,
    ProgramLiveEvents events, ILogger<ProgramConversationEndpoint> logger)
{
    ...
    public async Task RunAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            if (existing is null)
            {
                ...
                {
                    try { await programs.DeployAsync(ProgramExamples.IntoChat, 0, cancellationToken); }
    ...
    private static void RequireBinding(ProgramRunSnapshot run, string threadId, string hash, List<ChatMessage> supplied)
    {
        ...
        var matches = run.Input.TryGetProperty("requestHash", out var original)
            ...
            : run.Input.TryGetProperty("messages", out var messages)
                && ProgramConversationHistory.Read(messages).Where(message => message.Role == ChatRole.User)
