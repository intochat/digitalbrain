...
internal interface IConversation : IGrainWithStringKey
{
    ...
    Task CompleteRun(ProgramRunSnapshot run);
}
    ...
    : Grain, IConversation
{
    ...
    public async Task<JsonElement> PrepareInput(string requestHash, JsonElement suppliedMessages)
    {
        ...
        var messages = ProgramConversationHistory.Read(document.RootElement);
        messages.AddRange(ProgramConversationHistory.Read(suppliedMessages));
    ...
    public async Task CompleteRun(ProgramRunSnapshot run)
    {
        ...
        if (run.ProgramId != "intochat" || run.Input.ValueKind != JsonValueKind.Object
        ...
        {
            messages = ProgramConversationHistory.Read(run.Input.GetProperty("messages"));
            messages.Add(new ChatMessage(ChatRole.Assistant, ProgramConversationHistory.Answer(run.Output)));
        }
