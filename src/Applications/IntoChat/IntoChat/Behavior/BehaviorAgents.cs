...
public sealed class BehaviorAgents(IGrainFactory grains) : IBehaviorRunLifecycle
{
    private static readonly JsonSerializerOptions Json = new(JsonSerializerDefaults.Web);
    private const int MaximumAgents = 16;

    public async Task<JsonElement> ExecuteAsync(BehaviorExecutionContext context, JsonElement config, CancellationToken cancellationToken)
    {
        ...
        if (mode == "spawn")
        {
            if (config.TryGetProperty("items", out var items))
            {
                ...
                {
                    var itemConfig = BehaviorExpressions.Resolve(context.Node.Config, context.Input, item, context.Outputs);
    ...
    private async Task<NeuronId> SpawnAsync(BehaviorExecutionContext context, JsonElement config, string suffix, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var builder = grains.GetGrain<IAgentBuilder>(BuilderName(context.BehaviorId, context.RunId));
        ...
        var reference = await builder.Build(new(Command(context, "build/" + suffix), key,
            ...
            ReadText(config, "prompt"), ReadText(config, "name"), $"program:{context.BehaviorId}/{context.RunId}", retained));
    ...
    private async Task<JsonElement> ExecuteOnAgentAsync(BehaviorExecutionContext context, JsonElement config, JsonElement reference,
    ...
    public async Task FinishedAsync(BehaviorDefinition definition, BehaviorRunSnapshot run, CancellationToken cancellationToken)
    {
        ...
        var builder = grains.GetGrain<IAgentBuilder>(BuilderName(run.BehaviorId, run.RunId));
        await builder.Close(new(StableCommand($"{run.BehaviorId}/{run.RunId}/close")));
    }
    ...
    private static CommandId Command(BehaviorExecutionContext context, string operation)
        => StableCommand($"{context.BehaviorId}/{context.RunId}/{context.Version}/{context.Node.Id}/{operation}");
