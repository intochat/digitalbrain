...
internal sealed class ScopedBehaviorTools(IDigitalBrain brain, ContractCatalog catalog, string scope, bool allowActivation, BehaviorCatalogStore? registry = null)
{
    ...
    [McpServerTool(Name = "code_contracts"), Description("Discover installed module IDs, neuron contracts and a single-source behavior template before authoring C#. Pass modules=[] first to discover installed modules, then select exact returned IDs (for example time and flutter). A behavior name is not a module ID.")]
    public ContractCatalogSnapshot Contracts(string[] modules) => catalog.Read(modules);
    [McpServerTool(Name = "behavior_describe"), Description("Set a behavior's display name, purpose, and declared trigger/effect descriptions for the Behaviors application. These descriptions are documentation, not execution evidence. Use expectedRevision=0 for new entries; read current details before updating.")]
    ...
    [McpServerTool(Name = "behavior_details"), Description("Read the display name, purpose and metadata revision of a behavior before changing its description.")]
