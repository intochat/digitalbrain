...
public sealed class ModuleOverrideTransportFacts
{
    [Fact]
    public void ExplicitDefaultIsNativeWindow()
    {
        var composition = new BrainCompositionBuilder().WithModule<FlutterModule>(flutter => flutter.RunDesktopApp()).Build();
    ...
    public void DeepSettingsSurviveModuleOverrideTransport()
    {
        ...
        var overrides = new CompositionOverrides()
            ...
            {
                PublicOrigin = new("http://localhost:1234/"),
                TokenEndpoint = new("http://localhost:1234/token"),
            }));
