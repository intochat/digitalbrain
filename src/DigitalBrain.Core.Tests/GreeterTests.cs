...
public sealed class GreeterTests(BrainTestClusters clusters) : NeuronTest<Greeter>(clusters)
{
    ...
    public async Task GreetRoundTrip()
    {
        ...
        var greeted = await session.AskAsync(new Greet("Ada"), ct);
        ...
        Assert.Equal(new SynapseRef(session.Id, askSaid.Position), replyHeard.Metadata.Answers);
        ...
        Assert.Equal(new SynapseRef(session.Id, askSaid.Position), replySaid.Metadata.Answers);
