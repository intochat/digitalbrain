...
| `DigitalBrain.Context` (+ `.Tests`) | Real-grain ino. `IContextNeuron` + Qdrant. Core-only contracts; grain in Kernel (or moving via Abstractions). |
| `DigitalBrain.Google` (+ `.Tests`) | Real-grain ino. `IGmailNeuron`/`IGoogleAuthNeuron`. Clients/factories in peer. Grain impls + logic extracted toward peer (post-redesign); per-user keys enforced. References Abstractions. |
| `DigitalBrain.Salesforce` (+ `.Tests`) | Real-grain ino. `ISalesforce*`. Similar extraction; per-user auth grains. |
