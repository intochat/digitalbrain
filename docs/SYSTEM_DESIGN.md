...
| `DigitalBrain.Demo.Runtime` | Demo/sample runtime layer — owns the surface demo pack source, signing helper, and live observability graph surface builder. References Core plus demo, pack, marketplace, and UI contract packages, but not Kernel, hosts, integrations, or runtime transport packages. |
...
| `DigitalBrain.Kernel` | The Orleans runtime substrate. `Microsoft.NET.Sdk.Web`. Owns core: embodiment (Foundry), LLM, Economics, Context, UI gateway, HA self-update, journals. Hosts core grains + Abstractions base. References peer integrations for interfaces; concrete auth/integration grains moving to peers via Abstractions layer (see redesign 2026-07). |
...
| `DigitalBrain.Context` (+ `.Tests`) | Real-grain ino. `IContextNeuron` + Qdrant. Core-only contracts; grain in Kernel. |
| `DigitalBrain.Google` (+ `.Tests`) | Real-grain ino. `IGmailNeuron`/`IGoogleAuthNeuron`. Clients/factories in peer. Grain impls moved to peer (2026-07 redesign); per-user keys enforced. References Abstractions. |
| `DigitalBrain.Salesforce` (+ `.Tests`) | Real-grain ino. `ISalesforce*`. Grain impls moved to peer; per-user auth grains. References Abstractions. |
...
The split: `Neuron`/`NeuronJournals` base extracted to `DigitalBrain.Kernel.Abstractions` (2026-07 redesign) so peer integrations can own grains without full Kernel dep or cycles. Kernel hosts core + Abstractions. Integrations (Google, Salesforce) now reference Abstractions + provide their grains/impls in peer assemblies (Ino core orchestrator remains in Kernel due to tight coupling to cross-cutting like LLM/Context/Market). Per-user grain keys for auth (no more global *-main for user flows). Pure packs remain IPackBehavior. Ino orchestrator stays "ino-main" as core assistant; its connectors use per-user scopes/keys.

Progress: Google and Salesforce grains/impls moved to peers; surfaces, seeders, auth flows updated. Reflection used temporarily in some surface construction to bypass sub-namespace resolution during transition. All targeted tests green, full build green. Abstractions + per-user routing complete for auth.
...
each isolated project gets its own fast, direct grain-resolution tests without routing through the central
`DigitalBrain.Tests` project (which remains the broadest cross-cutting E2E safety net).
