...
| `DigitalBrain.Kernel` | The Orleans runtime substrate. `Microsoft.NET.Sdk.Web`. Owns core: embodiment (Foundry), LLM, Economics, UI gateway, HA self-update, journals. Hosts core grains + Abstractions base. References peer integrations for interfaces; integration grains (Google, Salesforce, Ino, Context) live in peer projects. |
...
| `DigitalBrain.Google` (+ `.Tests`) | Real-grain ino. `IGmailNeuron`/`IGoogleAuthNeuron`. Clients/factories in peer. Grain impls in peer; per-user keys enforced. References Abstractions. |
...
The split: `Neuron`/`NeuronJournals` base extracted to `DigitalBrain.Kernel.Abstractions` so peer integrations can own grains without full Kernel dep or cycles. Kernel hosts core + Abstractions. Integrations (Google, Salesforce, Ino) now reference Abstractions + provide their grains/impls in peer assemblies. Ino (intelligent personal assistant) treated as first-party integration like Google/Salesforce. Per-user grain keys for auth (no more global *-main for user flows). Pure packs remain IPackBehavior. Ino orchestrator stays "ino-main" as core assistant; its connectors use per-user scopes/keys.

Progress: integration grains/impls moved to peers (Google, Salesforce, Ino); surfaces, seeders, auth flows updated. All targeted tests green, full build green. Abstractions + per-user routing complete for auth.
