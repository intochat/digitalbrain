...
- **D.** `JournalJsonContext` **deleted** — Orleans.Journaling reconfigured to use its native binary format (every `Synapse` is already `[GenerateSerializer]`-annotated for Orleans' own automatic serializer; the JSON registry is a redundant parallel system), pending a bounded spike to confirm the exact configuration surface on this alpha API.
...
### 2.4 D — Delete JournalJsonContext (spike-gated)

Every `Synapse` subtype already carries `[GenerateSerializer]` for Orleans' own automatic serializer — zero manual registration, already used for every grain-to-grain message in the system. `JournalJsonContext`'s 126-entry manual list exists only because Orleans.Journaling is currently configured for `UseJsonJournalFormat()`. Web-verified (Context7 had thin coverage of this alpha namespace, confirmed via search of Orleans' own docs/release notes): JSON Lines is the *default* Orleans.Journaling format, but `JournaledStateManagerOptions.JournalFormatKey = "orleans-binary"` selects Orleans' native binary format instead.

This is a real, specific, testable configuration surface — not a guess — but it's on an alpha API (`ORLEANSEXP005`) with thin first-party documentation, so this spec does **not** commit to exact code for the swap itself. It commits to a bounded spike (§3, first task) that confirms whether setting `JournalFormatKey = "orleans-binary"` on the actual installed `Microsoft.Orleans.Journaling` package version eliminates the need for any JSON-specific type registration. If confirmed: delete `JournalJsonContext` entirely and wire the binary format. If the alpha API doesn't support this cleanly yet: fall back to a Roslyn incremental source generator that scans the compilation for `Synapse`-derived types and auto-emits `JournalJsonContext`'s `[JsonSerializable]` list, so the manual step is at least made structurally impossible to forget rather than deleted outright.
