...
| Current task | Phase 2: spikes S1-S4 done (NOTES "Phase 2 spikes", design 4.4 amended twice: standby as an executable over its artifacts, lease in a dedicated table); Task 8 (the AppHost: gateway on 5080, kernel-a, explicit-start kernel-b) in progress; Tasks 1-7 complete |
| Last green gate | `9497b611` (2026-09-14): format + build 0 warnings + `dotnet test DigitalBrain.slnx` 505 passed, 13 skipped (4 Docker + 3 coding gated + 6 lease gated), 0 failed; gated slot build 1/1 into artifacts/slot-b at `db0e0c81` |
| Last commit | `9497b611` coding: the gateway bounds its lease read and canonicalises slot names |
...
| 2 slots | `feature/coding-phase2-slots` | in progress (task 8 of 11) | — |
| 3 swarm | — | not started | — |
