...
public sealed class UiHttpRoundTrip(UiEdgeFixture fixture)
{
    ...
    public async Task HttpOpenSceneJournalsSceneOpened()
    {
        ...
        using var response = await http.PostAsJsonAsync(
            UiHttpSse.OpenScene(UiEdgeFixture.DefaultShellName),
    ...
    public async Task HttpShellEventsRejectsNegativeAfterSequence()
    {
        ...
        using var response = await http.GetAsync(
            new Uri(UiHttpSse.ShellEvents(UiEdgeFixture.DefaultShellName, afterSequence: -1), UriKind.Relative),
            cancellationToken);
    ...
    public async Task HttpChatTurnCarriesPulseIdentity()
    {
        ...
        Assert.Equal(UiEdgeAssistantProbe.Answer, answered.Text);
    ...
    private static async Task<ShellEventStream> OpenShellEventStreamAsync(HttpClient http, CancellationToken cancellationToken)
    {
        using var streamRequest = new HttpRequestMessage(
            HttpMethod.Get,
            UiHttpSse.ShellEvents(UiEdgeFixture.DefaultShellName, afterSequence: 0));
