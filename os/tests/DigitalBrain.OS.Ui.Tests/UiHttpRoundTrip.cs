...
public sealed class UiHttpRoundTrip(FlutterHttpFixture fixture)
{
    ...
    public async Task HttpOpenSceneJournalsSceneOpened()
    {
        ...
        using var response = await http.PostAsJsonAsync(
            UiHttpSse.OpenScene(FlutterHttpFixture.DefaultShellName),
    ...
    public async Task HttpShellEventsRejectsNegativeAfterSequence()
    {
        ...
        using var response = await http.GetAsync(
            new Uri(UiHttpSse.ShellEvents(FlutterHttpFixture.DefaultShellName, afterSequence: -1), UriKind.Relative),
            cancellationToken);
    ...
    public async Task HttpChatTurnCarriesPulseIdentity()
    {
        ...
        Assert.Equal(FlutterHttpAssistantProbe.Answer, answered.Text);
    ...
    private static async Task<ShellEventStream> OpenShellEventStreamAsync(HttpClient http, CancellationToken cancellationToken)
    {
        using var streamRequest = new HttpRequestMessage(
            HttpMethod.Get,
            UiHttpSse.ShellEvents(FlutterHttpFixture.DefaultShellName, afterSequence: 0));
        ...
        Assert.Contains(
            FlutterHttpContract.CacheControlNoCache,
