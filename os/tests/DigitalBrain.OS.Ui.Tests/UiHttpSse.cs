...
internal static class UiHttpSse
{
    ...
    public static string OpenScene(string shellName) =>
        FlutterHttpContract.OpenScenePath.Replace("{shellName}", shellName, StringComparison.Ordinal);

    public static string ShellEvents(string shellName, long afterSequence = 0) =>
        $"{FlutterHttpContract.ShellEventsPath.Replace("{shellName}", shellName, StringComparison.Ordinal)}?{FlutterHttpContract.AfterSequenceQuery}={afterSequence}";
    ...
    public static string ChatEvents(string chatName, long afterSequence = 0) =>
        $"{FlutterHttpContract.ChatEventsPath.Replace("{chatName}", chatName, StringComparison.Ordinal)}?{FlutterHttpContract.AfterSequenceQuery}={afterSequence}";
        ...
        TimeSpan? timeout = null)
    {
        ...
        while (!linked.IsCancellationRequested)
        {
            ...
            if (line.Length == 0 && dataLine is not null)
            {
                ...
                if (name is not null
                    && !string.Equals(name, FlutterHttpContract.SceneOpenedEvent, StringComparison.Ordinal))
                {
                    continue;
                }
    ...
    public static async Task<ChatTurnEvent> ReadNextChatTurnAsync(
        ...
        CancellationToken cancellationToken)
    {
        var payload = await ReadNextEventPayloadAsync(reader, FlutterHttpContract.ChatTurnEvent, cancellationToken);
