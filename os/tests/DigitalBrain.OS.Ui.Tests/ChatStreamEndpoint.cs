...
public sealed class ChatStreamEndpoint(FlutterHttpFixture fixture)
{
    ...
    public async Task StreamEndpointYieldsDeltasAndJournalsOneAnswer()
    {
        ...
        Assert.Equal(
            FlutterHttpAssistantProbe.Answer,
        ...
        Assert.Collection(
            ...
            fact => Assert.Equal(FlutterHttpAssistantProbe.Answer, Assert.IsType<AssistantResponded>(fact.Synapse).Text));
    }
    ...
    [Fact(DisplayName =
        "FlutterHttpContract names the stream path and chat-delta event; vocabulary stays closed")]
        ...
        CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(
            HttpMethod.Post,
            FlutterHttpContract.StreamMessagePath.Replace("{chatName}", chatName, StringComparison.Ordinal))
        {
        ...
        Assert.Equal(
            FlutterHttpContract.EventStreamContentType,
        ...
        while (!headerBudget.IsCancellationRequested)
        {
            ...
            {
                if (string.Equals(eventName, FlutterHttpContract.ChatDeltaEvent, StringComparison.Ordinal))
                {
