...
public sealed class AuthorizationProjection(FlutterHttpFixture fixture)
{
    ...
    [Fact(DisplayName = "FlutterHttpContract names the authorization events path and event")]
        ...
        CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"{FlutterHttpContract.AuthorizationEventsPath}?{FlutterHttpContract.AfterSequenceQuery}={afterSequence}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (!linked.IsCancellationRequested)
        {
            ...
            if (line.Length == 0
                && string.Equals(eventName, FlutterHttpContract.AuthorizationEvent, StringComparison.Ordinal)
