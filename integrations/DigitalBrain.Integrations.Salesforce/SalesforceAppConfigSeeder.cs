...
internal sealed class SalesforceAppConfigSeeder(IConfiguration configuration, IIntegrationConfigStore store, ILogger<SalesforceAppConfigSeeder> logger)
    : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        var existing = await store.GetAsync(SalesforceClientFactory.DefaultScope, SalesforceClientFactory.PackName, cancellationToken).ConfigureAwait(false);
        ...
        changed |= SetIfConfigured(
            ...
            string.IsNullOrWhiteSpace(appConfig.LoginUrl) ? SalesforceClientFactory.DefaultLoginUrl : appConfig.LoginUrl);
        changed |= SetIfConfigured(
            merged,
            SalesforceClientFactory.ApiVersionKey,
            string.IsNullOrWhiteSpace(appConfig.ApiVersion) ? SalesforceClientFactory.DefaultApiVersion : appConfig.ApiVersion);
        changed |= SetIfConfigured(merged, SalesforceClientFactory.RedirectUriKey, appConfig.RedirectUri);
        if (!changed)
        {
            return;
        }
        await store.SetAsync(SalesforceClientFactory.DefaultScope, SalesforceClientFactory.PackName, merged, cancellationToken).ConfigureAwait(false);
    ...
    private sealed record SalesforceAppConfig(string? ClientId, string? ClientSecret, string? LoginUrl, string? ApiVersion, string? RedirectUri)
    {
