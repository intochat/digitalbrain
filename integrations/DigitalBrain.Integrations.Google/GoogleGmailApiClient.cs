...
internal sealed class GoogleGmailApiClient : IGmailApiClient
{
    ...
    public async Task<ContractGmailMessage> ReadMessageAsync(GmailMessageReadRequest request, CancellationToken cancellationToken = default)
    {
        ...
        get.Fields = "id,threadId,internalDate,payload";
        var message = await get.ExecuteAsync(cancellationToken).ConfigureAwait(false);
        var sender = ParseSender(SingleHeader(message, "From"));
        return new ContractGmailMessage(
            message.Id,
            message.ThreadId,
    ...
    public async Task<GmailMailboxPage> ReadMailboxAsync(GmailMailboxReadRequest request, CancellationToken cancellationToken = default)
    {
        var offset = request.ContinuationToken is null
            ? 0
            : int.TryParse(request.ContinuationToken, out var parsed) && parsed >= 0
                ? parsed
                : throw new ArgumentException("The Gmail continuation token is invalid.", nameof(request));
        var result = await ListMessagesAsync(new GmailMessageListRequest(new GmailMessageSelection(Mailbox: GmailMailboxScope.Inbox), offset, request.Limit), cancellationToken)
            .ConfigureAwait(false);
        var messages = result.Messages.Select(message => new GmailMessageSummary(message.MessageId, message.ThreadId, DateTimeOffset.FromUnixTimeMilliseconds(message.InternalDate), message.FromAddress, message.Subject)).ToArray();
    ...
    public async Task<GmailMessageListResult> ListMessagesAsync(GmailMessageListRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (stableCandidateIds is not null)
        {
            ...
            messages = pageSelection.PinnedMessageIds!.Where(messagesById.ContainsKey).Select(id => messagesById[id])
                .ToArray();
        }
    ...
    public async Task<GmailThreadListResult> ListThreadsAsync(GmailThreadListRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var stableThreads = window.Messages.Where(static message => !string.IsNullOrWhiteSpace(message.ThreadId))
            ...
            .Select(group =>
            {
                var messages = group.OrderByDescending(static message => message.InternalDate)
                    .ThenBy(static message => message.MessageId, StringComparer.Ordinal)
                    .ToArray();
                var participants = messages.SelectMany(static message => message.FromAddress is null ? message.ToAddresses : message.ToAddresses.Prepend(message.FromAddress))
            ...
            .ToArray();
        var threads = stableThreads.Skip(request.Offset).Take(request.Limit).ToArray();
    ...
    private static bool Matches(GmailMessageSelection selection, GmailMessageMetadata message)
    {
        ...
        var mailboxMatch = selection.Mailbox switch
        {
            ...
            GmailMailboxScope.Inbox => labels.Contains("INBOX", StringComparer.Ordinal) && !HasAny(labels, "SENT", "DRAFT", "SPAM", "TRASH"),
        ...
        return mailboxMatch &&
               (selection.ReadState == GmailMessageReadState.Any || selection.ReadState == GmailMessageReadState.Read && message.IsRead ||
                selection.ReadState == GmailMessageReadState.Unread && !message.IsRead) &&
               (selection.SenderAddress is null || string.Equals(selection.SenderAddress, message.FromAddress, StringComparison.OrdinalIgnoreCase)) &&
               (selection.RecipientAddress is null || message.ToAddresses.Contains(selection.RecipientAddress, StringComparer.OrdinalIgnoreCase)) &&
               (selection.SubjectContains is null || message.Subject?.Contains(selection.SubjectContains, StringComparison.OrdinalIgnoreCase) == true) &&
    ...
    private static byte[] Rfc2822(GmailSendRequest request, string messageId)
    {
        ...
        var encodedBody = Convert.ToBase64String(Encoding.UTF8.GetBytes(normalizedBody), Base64FormattingOptions.InsertLineBreaks);
    ...
    private static string PlainText(MessagePart? part)
    {
        ...
        {
            var encoded = part.Body.Data.Replace('-', '+').Replace('_', '/');
            encoded = encoded.PadRight((encoded.Length + 3) / 4 * 4, '=');
            return Encoding.UTF8.GetString(Convert.FromBase64String(encoded));
        }
        foreach (var child in part.Parts ?? [])
        {
            var text = PlainText(child);
    ...
    private static string[] ParseAddresses(string? value)
    {
        ...
        try
        {
            ...
            addresses.Add(value);
            return addresses.Cast<MailAddress>().Where(static address => !string.IsNullOrWhiteSpace(address.User) && !string.IsNullOrWhiteSpace(address.Host))
    ...
    private static byte[] DecodeQuotedPrintableWord(string value)
    {
        ...
        for (var index = 0; index < value.Length; index++)
        {
            ...
            if (current == '=' && index + 2 < value.Length &&
                byte.TryParse(value.AsSpan(index + 1, 2), System.Globalization.NumberStyles.HexNumber, System.Globalization.CultureInfo.InvariantCulture, out var decoded))
            {
                bytes.Add(decoded);
                index += 2;
                continue;
...
internal static class GmailSendRequestValidator
{
    ...
    public static void Validate(GmailSendRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(recipient) || recipient.Length > GmailTools.MaximumRecipientLength || recipient.Any(char.IsControl) ||
