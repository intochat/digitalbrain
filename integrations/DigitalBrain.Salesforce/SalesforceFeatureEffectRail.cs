...
public sealed record SalesforceFeatureEffectPayload(
    int Version,
    ...
    DateTimeOffset ExpiresAt)
{
    ...
    public static SalesforceFeatureEffectPayload Create(
...
public sealed record SalesforceFeatureEffectRequest(
...
public sealed record SalesforceFeatureEffectProposal(
...
public sealed class SalesforceFeatureEffectRail(
    IFeatureGrainResolver grains,
    IInoEffectPlanStore plans,
    IInoEffectExecutor effects,
    TimeProvider timeProvider)
{
    public const string OutcomeKind = "salesforce.record.update.outcome.v1";

    public async Task<SalesforceFeatureEffectProposal> ProposeAsync(
        SalesforceFeatureEffectRequest request,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);
        ValidateRequest(request);
        var persistedOperationKey = FeatureIntentKeys.Create(
            request.InstallationId,
            request.InputId,
            request.LogicalOperationKey);
        var installation = grains.Installation(request.OwnerId, request.InstallationId);
        var intent = (await installation.ListPendingIntentsAsync().WaitAsync(cancellationToken))
        ...
        var payload = SalesforceFeatureEffectPayload.Parse(intent.PayloadJson);
        ...
        var approval = await plans.PrepareIdempotentAsync(
            identity,
            actorScope,
            operationId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await effects.ExecuteAsync(new InoToolEffectRequest(
            proposal.OperationId,
            proposal.EffectId,
            SalesforceTools.UpdateRecord,
            proposal.Approval.Scope,
            proposal.ActorScope,
            proposal.ProviderIdempotencyKey), cancellationToken);
        var request = proposal.Request;
        await grains.Installation(request.OwnerId, request.InstallationId)
            .ApplyIntentAsync(proposal.PersistedOperationKey)
