...
public sealed record GmailMessageReceived(
...
public interface IGmailWatchEventHandler
{
    Task<FeatureFanOutResult> HandleAsync(
        GmailMessageReceived message,
        CancellationToken cancellationToken = default);
}

public sealed class GmailWatchEventHandler(IFeatureGrainResolver grains) : IGmailWatchEventHandler
{
    public const string EventKind = "gmail.message.received.v1";

    public Task<FeatureFanOutResult> HandleAsync(
        GmailMessageReceived message,
        CancellationToken cancellationToken = default)
    {
        ...
        var payload = JsonSerializer.Serialize(new GmailMessageFacts(
            message.MessageId,
            message.ThreadId,
            message.HistoryId));
