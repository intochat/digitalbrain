...
public static class Extensions
{
    ...
    public static TBuilder ConfigureOpenTelemetry<TBuilder>(this TBuilder builder) where TBuilder : IHostApplicationBuilder
    {
        ...
        builder.Services.AddOpenTelemetry().WithMetrics(metrics =>
            ...
            .WithTracing(tracing =>
            {
                tracing.AddSource(builder.Environment.ApplicationName).AddSource("Microsoft.Orleans.Application").AddSource("DigitalBrain.Neuron")
    ...
    private static Task WriteSafeHealthResponseAsync(HttpContext context, HealthReport report)
    {
        ...
        var payload = new Dictionary<string, object?> { ["status"] = report.Status.ToString() };
        if (report.Entries.TryGetValue("digitalbrain-runtime-state", out var runtimeState))
        {
            var safe = new Dictionary<string, object?> { ["status"] = runtimeState.Status.ToString() };
            foreach (var key in new[] { "backendKind", "namespace", "schemaVersion", "keyVersion" })
