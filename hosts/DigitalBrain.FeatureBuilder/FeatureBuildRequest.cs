    ...
    public FeatureSourceFile(string path, string content)
    {
        ...
        if (byteCount > FeatureSourceSnapshot.MaximumFileBytes)
        {
            throw new ArgumentException(
                $"A source file cannot exceed {FeatureSourceSnapshot.MaximumFileBytes} UTF-8 bytes.",
                nameof(content));
        }
...
public sealed class FeatureSourceSnapshot
{
    ...
    public FeatureSourceSnapshot(
        string implementationProjectPath,
        string scenarioProjectPath,
        IReadOnlyList<FeatureSourceFile> files)
    {
        ImplementationProjectPath = ValidatePath(
            implementationProjectPath,
            nameof(implementationProjectPath));
        ...
        {
            throw new ArgumentException(
                $"A source snapshot must contain 1 to {MaximumFileCount} files.",
                nameof(files));
        }
        ...
        {
            var file = files[index] ?? throw new ArgumentException(
                "Source snapshots cannot contain null files.",
                nameof(files));
            ...
            {
                throw new ArgumentException(
                    $"A source snapshot cannot exceed {MaximumTotalBytes} UTF-8 bytes.",
                    nameof(files));
            }
    ...
    internal static string ValidatePath(string path, string parameterName)
    {
        ...
        if (path.Length is 0 or > MaximumPathLength ||
...
public sealed class FeatureBuildRequest
{
    ...
    private static bool PathsOverlap(string first, string second)
    {
        ...
        return string.Equals(first, second, comparison) ||
            firstPrefix.StartsWith(secondPrefix, comparison) ||
