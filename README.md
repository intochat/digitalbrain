...
Southbound MCP transport lives in `DigitalBrain.Sdk/Mcp` (Salesforce and shared OAuth rail).
Gmail calls Gmail REST through `Google.Apis.Gmail.v1` with a reflected read-only planner catalog.
Northbound agent tools live in process `DigitalBrain.Mcp` (`AddDigitalBrainClient`). Docs that say
MCP is co-hosted on the silo are stale.
