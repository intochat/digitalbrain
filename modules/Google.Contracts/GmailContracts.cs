...
public sealed class GmailSendExecutionRequest
{
    public GmailSendExecutionRequest(string effectKey)
    {
        EffectKey = ContractGuard.Required(effectKey, nameof(effectKey), 512);
    }

    public string EffectKey { get; }
}

public sealed class GmailSendResult
{
    public GmailSendResult(string digest, string providerMessageId)
    {
        Digest = ContractGuard.Required(digest, nameof(digest), 512);
        ProviderMessageId = ContractGuard.Required(providerMessageId, nameof(providerMessageId), 512);
    }
