...
internal sealed class TestReminderDriver
{
    ...
    internal TestReminderDriver(VolatileReminderTable table, IGrainFactory grains, string scope)
    {
        _table = table;
        _scope = scope;

        var caller = NeuronId.For<ITestReminderDeliveryCaller>(new OwnerId($"{scope}-reminder-driver"), "caller");
        _caller = grains.GetGrain<ITestReminderDeliveryCaller>(caller.ToGrainId());
    }

    internal DateTimeOffset? NextDueAtOrBefore(DateTimeOffset target)
        => _table.NextDueAtOrBefore(target, _scope)?.Due;

    internal async Task<bool> TryDeliverNextDueAsync(DateTimeOffset now, CancellationToken cancellationToken)
    {
