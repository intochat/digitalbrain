...
{
    [Fact]
    public async Task Storage_nests_under_Core_under_Modules()
    {
        var builder = DistributedApplication.CreateBuilder([]);
        builder.AddDigitalBrain("Modules").AddModule<SupabaseModule>();

        Assert.Equal("Modules", ParentName(builder.Resources.OfType<DigitalBrainModuleResource>().Single(resource => resource.Name == "Core")));
        Assert.Equal("Core", ParentName(builder.Resources.Single(resource => resource.Name == "storage")));
