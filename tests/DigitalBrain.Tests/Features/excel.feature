Feature: Excel
  ...
  Scenario: A replacement grid is normalised on apply
    ...
    When "claude" applies a replacement to sheet "replacement" with blank title and sheet name and ragged rows
      ...
      | Tea  |          |       |
      | Cake | 2        | Fresh |
    Then the spreadsheet command is accepted against version 0
  ...
  Scenario: Two rapid cell edits both survive
    Given a running brain
    And "sheet:racing" is connected to "claude" for "SheetChanged"
    When "claude" applies cell edits to sheet "racing" concurrently, "Tea" at row 0 and column 0 and "Cake" at row 1 and column 1
    Then "claude" waits up to 5 seconds for 2 incoming "SheetChanged"
    And "claude" incoming journal contains "SheetChanged" {"name":"racing","title":"Sheet","version":1}
    And "claude" incoming journal contains "SheetChanged" {"name":"racing","title":"Sheet","version":2}
    And "claude" waits up to 5 seconds until sheet "racing" has 2 rows and 2 columns with value "Tea" at row 0 and column 0
    And "claude" waits up to 5 seconds until sheet "racing" has 2 rows and 2 columns with value "Cake" at row 1 and column 1
