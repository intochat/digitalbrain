...
public sealed class V2SemanticIntentModelTests
{
    ...
    public async Task Resolve_intent_uses_separate_roles_json_schema_and_descriptor_only_grounding()
    {
        ...
        var grounding = new V2GroundingDescriptor(
            Provider: "gmail",
        ...
        await grain.ResolveIntentAsync(
            new V2SemanticIntentRequest(tenantId, workspaceId, conversationId, prompt, [grounding]));
    ...
    public async Task Resolve_intent_deserializes_typed_proposals_for_typos_and_paraphrases(string prompt)
    {
        ...
        Assert.Equal(V2SemanticProvider.Gmail, proposal.Provider);
        Assert.Equal(V2SemanticOperation.List, proposal.Operation);
        Assert.Equal("message", proposal.Entity);
        Assert.Equal(2, proposal.Limit);
        Assert.Equal(2, proposal.Ordinal);
        Assert.Equal(V2SemanticReference.LatestProviderResult, proposal.Reference);
        Assert.Equal(V2SemanticTimeRange.PreviousWeek, proposal.TimeRange);
        var filter = Assert.Single(proposal.Filters!);
        Assert.Equal(new V2SemanticFilter("readState", V2SemanticFilterOperator.Equals, "unread"), filter);
        var sort = Assert.Single(proposal.Sorts!);
        Assert.Equal(new V2SemanticSort("receivedTime", V2SemanticSortDirection.Descending), sort);
    ...
    private static V2SemanticIntentRequest Request(string prompt) =>
