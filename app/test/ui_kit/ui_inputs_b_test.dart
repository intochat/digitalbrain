...
void main() {
  ...
  testWidgets('RadioGroup capture: tapping option writes value to form scope', (
    tester,
  ) async {
  ...
  testWidgets('Slider capture: drag writes numeric string to form scope', (
    tester,
  ) async {
    final controller = UiKitFormController();
    await tester.pumpWidget(
    ...
    final startPt =
        sliderTopLeft + Offset(sliderSize.width * 0.1, sliderSize.height / 2);
  ...
  testWidgets(
    'DateField capture: text entry writes ISO yyyy-MM-dd string to form scope',
    (tester) async {
      final controller = UiKitFormController();
      await tester.pumpWidget(
        MaterialApp(
