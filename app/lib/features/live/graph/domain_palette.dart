...
final Map<String, DomainStyle> domainPalette = {
  'kernel': DomainStyle(
    ...
    anchor: vm.Vector3(0.00, 0.20, 1.05) * _kSphereRadius,
    shortLabel: 'KERNEL',
  ),
  'ai': DomainStyle(
    color: const Color(0xFF7C8AFF),
    anchor: vm.Vector3(0.95, 0.55, 0.10) * _kSphereRadius,
    shortLabel: 'AI',
  ),
  'google': DomainStyle(
    color: const Color(0xFFE8C56A),
    anchor: vm.Vector3(-0.85, 0.65, 0.40) * _kSphereRadius,
    shortLabel: 'GOOGLE',
  ),
  'data': DomainStyle(
    color: const Color(0xFF6EE7A8),
    anchor: vm.Vector3(0.60, -0.65, 0.55) * _kSphereRadius,
    shortLabel: 'DATA',
  ),
  'travel': DomainStyle(
    color: const Color(0xFF9CB3FF),
    anchor: vm.Vector3(-0.55, -0.55, 0.70) * _kSphereRadius,
    shortLabel: 'TRAVEL',
  ),
  'dynamic': DomainStyle(
    color: const Color(0xFFF4B8E4),
    anchor: vm.Vector3(0.25, -0.95, -0.30) * _kSphereRadius,
    shortLabel: 'DYNAMIC',
  ),
  'system': DomainStyle(
    color: const Color(0xFFB8C5E0),
    anchor: vm.Vector3(0.05, 0.95, -0.45) * _kSphereRadius,
    shortLabel: 'SYSTEM',
  ),
};
...
Color colorForSynapseType(String typeFullName) {
  ...
  if (s.contains('.Llm') ||
      s.contains('Classify') ||
      s.contains('Voice2Text') ||
      s.contains('InnoLang')) {
    return _periwinkle;
  }
  if (s.contains('Store') || s.contains('Sqlite') || s.contains('Database')) {
    return _mint;
  }
  if (s.contains('Gmail') || s.contains('OAuth')) return _gold;
  if (s.contains('TripPlan') || s.contains('.Word')) return _softBlue;
  if (s.contains('CreateNeuron') ||
      s.contains('NeuronCreated') ||
      s.contains('.Diagram')) {
    return _magenta;
  }
  return _neutral;
...
String shortNeuronLabel(String typeFullName) {
  ...
  return tail.endsWith('Neuron')
      ? tail.substring(0, tail.length - 'Neuron'.length)
      : tail;
}
