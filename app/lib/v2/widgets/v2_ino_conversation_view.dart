...
import 'v2_ino_composer.dart';

const Key v2InoConversationKey = Key('v2-ino-conversation');
const Key v2InoIntroKey = Key('v2-ino-intro');
const Key v2InoTranscriptKey = Key('v2-ino-transcript');
...
const Key v2InoSubmissionNoticeKey = Key('v2-ino-submission-notice');

typedef V2InoActionSubmit =
...
class V2InoConversationView extends StatefulWidget {
  const V2InoConversationView({
    super.key,
  ...
  final V2InoActionSubmit onSubmitAction;
  final bool actionEnabled;
  final bool reconnecting;
  final bool connectionUnavailable;

  @override
  State<V2InoConversationView> createState() => _V2InoConversationViewState();
}

class _V2InoConversationViewState extends State<V2InoConversationView> {
  ...
  @override
  void didUpdateWidget(covariant V2InoConversationView oldWidget) {
  ...
  bool get _canSend =>
      ...
      _composer.text.trim().length <= v2InoMaximumPromptLength;
  ...
  static bool _definitelyNotSubmitted(Object error) {
    ...
    return error is V2TransportException &&
        (error.code == V2TransportErrorCode.unauthenticated ||
            error.code == V2TransportErrorCode.permissionDenied ||
            error.code == V2TransportErrorCode.invalidArgument);
  }
...
class _ConnectionUnavailableBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    ...
    child: Material(
      key: v2InoConnectionUnavailableBannerKey,
