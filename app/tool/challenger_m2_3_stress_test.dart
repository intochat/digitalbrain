...
void main() {
  ...
  try {
    ...
    final paintMethodStart = content.indexOf(
    ...
    if (paintMethodStart == -1 || paintMethodEnd == -1) {
      ...
      final paintAllocations = RegExp(
        r'Paint\(\)',
      ).allMatches(paintBody).length;
      final pathAllocations = RegExp(r'Path\(\)').allMatches(paintBody).length;
      final textPainterAllocations = RegExp(
        r'TextPainter\(\)',
      ).allMatches(paintBody).length;
      final textSpanAllocations = RegExp(
        r'TextSpan\(\)',
      ).allMatches(paintBody).length;
      final textLayoutCalls = RegExp(
        r'\.layout\(',
      ).allMatches(paintBody).length;
    ...
    assertTest(
      'Static analysis of brain_canvas_2d_graph.dart completed without errors: $e',
      false,
    );
  }
  ...
  try {
    final file = File(
      'lib/features/neuron_constructor/neuron_constructor_view.dart',
    );
    final content = file.readAsStringSync();

    // GridPainter inspection
    final gridStart = content.indexOf(
      'class GridPainter extends CustomPainter',
    );
    final gridEnd = content.indexOf('class CablePainter', gridStart);
    final gridBody = content.substring(gridStart, gridEnd);
    final gridPaintAllocations = RegExp(
      r'Paint\(\)',
    ).allMatches(gridBody).length;
    ...
    final cableStart = content.indexOf(
      'class CablePainter extends CustomPainter',
    );
    final cableBody = content.substring(cableStart);

    final cablePaintAllocations = RegExp(
      r'Paint\(\)',
    ).allMatches(cableBody).length;
    final cablePathAllocations = RegExp(
      r'Path\(\)',
    ).allMatches(cableBody).length;
    ...
    final cableShouldRepaintAlways = cableBody.contains(
      'bool shouldRepaint(covariant CablePainter oldDelegate) => true;',
    );
    stdout.writeln(
      '  - shouldRepaint returns true always: $cableShouldRepaintAlways',
    );
    ...
    assertTest(
      'Static analysis of neuron_constructor_view.dart completed without errors: $e',
      false,
    );
  }
  ...
  final dragReleasePos = [
    150.0 * 50 + 0,
    50.0,
  ]; // near node_50_in (port offset x=0, y=50)
  ...
  for (int run = 0; run < 1000; run++) {
    ...
    final currentPos = [
      dragReleasePos[0] + 5.0,
      dragReleasePos[1] + 5.0,
    ]; // close to target
    ...
    for (var node in nodes) {
      ...
      for (var portId in node.ports) {
        ...
        final dist =
            dx * dx + dy * dy; // squared distance to avoid expensive sqrt
  ...
  stdout.writeln(
    'Gesture Drag Release Port Search (1000 runs, avg per search): ${avgMsPerSearch.toStringAsFixed(6)} ms',
  );
  stdout.writeln(
    'Total search execution time for 1000 runs: ${totalMs.toStringAsFixed(3)} ms',
  );

  assertTest(
    'A single drag release port match executes under 0.1ms for 100 nodes',
    targetNodeId == 'node_50' &&
        targetPortId == 'node_50_in' &&
        avgMsPerSearch < 0.1,
  );
  ...
  for (var node in nodes) {
    ...
    for (final _ in outConns) {
      sb.writeln(
        '    emit signal(DigitalBrain.Custom.TargetNode)(summary: "forward")',
      );
    }
    sb.writeln();
  }
  ...
  stdout.writeln(
    'Code generation took ${codeGenStopwatch.elapsedMilliseconds} ms.',
  );
  assertTest(
    'Code generation scales and executes in < 20ms',
    codeGenStopwatch.elapsedMilliseconds < 20,
  );
