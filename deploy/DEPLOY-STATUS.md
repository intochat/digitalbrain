...
resources). Both kept resources and the kernel app are healthy.
...
- `azure-native:app:ContainerApp` digitalbrain-jobs (kernel, internal Http2 ingress on port 8080 / gRPC gateway)
...
### Image registry
The kernel image lives in **public Docker Hub**: `docker.io/vhorbachov/digitalbrain-kernel:v4`
...
The kernel app (`digitalbrain-jobs`) runs with Azure **Table clustering**
...
#   -p:ContainerRepository=vhorbachov/digitalbrain-kernel -p:ContainerImageTag=<tag>  then docker push.
```
...
2. **Rotate secrets** (if not already real values):
   - `gh secret set DOCKERHUB_TOKEN` (must allow push to vhorbachov/digitalbrain-kernel)
